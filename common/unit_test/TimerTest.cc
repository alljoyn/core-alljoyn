...
static bool testNextAlarm(const Timespec& expectedTime, void* context)
{
    ...
    if (!triggeredAlarms.empty()) {
        ...
        ret = (p.first == ER_OK) && (context == p.second->GetContext()) && (alarmTime + TIMESTAMP_GRANULARITY >= expectedTimeMs) && (alarmTime < (expectedTimeMs + jitter));
        if (!ret) {
...
TEST(TimerTest, TestMaxAlarms) {
    ...
    ASSERT_GE(delta2 + TIMESTAMP_GRANULARITY, t2);
