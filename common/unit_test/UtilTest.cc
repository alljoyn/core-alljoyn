...
TEST(UtilTest, windows_os_snprintf_test) {
    ...
    // nomral case: buffer is larger than input
    EXPECT_EQ(4, AJ_snprintf(buffer, 5, format, argp));
    EXPECT_STREQ(argp, buffer);
    EXPECT_EQ('\0', buffer[4]);

    // normal case: buffer is larger than copy length limit
    EXPECT_EQ(4, AJ_snprintf(buffer, 4, format, argp));
    EXPECT_STREQ("asd", buffer);
    EXPECT_EQ('\0', buffer[3]);

    // sepcial case: buffer is equal or smaller than input.
    EXPECT_EQ(5, AJ_snprintf(buffer, 5, format, "asdfg"));
