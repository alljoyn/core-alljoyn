...
static TEST_CASE testVector[] = {
    ...
    },
    {
        ...
        "00 8D 49 3B 30 AE 8B 3C 96 96 76 6C FA",
        42,
        "0b f8 9e 3b 7d b8 62 4b 5a 7d ec 72 fd 5a 32 36 9a 6b 84 fe 05 d7 4a 11 5e 4d 40 a9 05 5c 6c 0a d6 56 09 be ef f9 3d ba cc a5 50 6a f7 76 61 96 95 06 71 de f6 c7 0a 71 14 22 5f 15 35 bb 8d",
        1040,
        expectedBigData,
        10
    },
...
TEST(AES_CCMTest, AES_CCM_Test_Vector) {
    ...
    for (size_t i = 0; i < ArraySize(testVector); i++) {
        ...
        size_t inputLen = HexStringToBytes(testVector[i].input, input, sizeof(input), ' ');
        uint32_t repeat = testVector[i].repeat;
        size_t len = inputLen * repeat;
        msg = new uint8_t[len + testVector[i].authLen];
        for (uint32_t j = 0; j < repeat; j++) {
            memcpy(&msg[inputLen * j], &input[0], inputLen);
        }
        String original = BytesToHexString(msg, len, false, ' ');
        ...
        String encrypted = BytesToHexString(msg, len, false, ' ');
        EXPECT_STREQ(testVector[i].output, encrypted.c_str()) << "Encrypt verification failure for test #"
                                                              << (i + 1);
        ...
        EXPECT_EQ(ER_OK, status) << "Authentication failure " << QCC_StatusText(status)
                                 << " for test #" << (i + 1);

        String decrypted = BytesToHexString(msg, len, false, ' ');
        EXPECT_STREQ(original.c_str(), decrypted.c_str()) << "Decrypt verification failure for test #" << (i + 1);
