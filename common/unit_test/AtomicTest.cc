...
TEST(AtomicTest, Atomic_int32)
{
    ...
    ASSERT_EQ(int32_t(a++), int32_t(a) - 1);
    ASSERT_EQ(int32_t(a--), int32_t(a) + 1);

    ASSERT_EQ(int32_t(++a), int32_t(a));
    ASSERT_EQ(int32_t(--a), int32_t(a));
}
...
{
    atomic_uint32 a(10);
    ASSERT_EQ(uint32_t(a), 10);
    
    qcc::Thread t("dummy", my_thread_func_uint32);
    t.Start(&a);
    t.Join();
    ASSERT_EQ(uint32_t(a), 11);
    
    ASSERT_EQ(uint32_t(a++), uint32_t(a) - 1);
    ASSERT_EQ(int32_t(a--), uint32_t(a) + 1);

    ASSERT_EQ(uint32_t(++a), uint32_t(a));
    ASSERT_EQ(uint32_t(--a), uint32_t(a));
}
