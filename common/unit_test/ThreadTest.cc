...
static const uint32_t s_multipleThreadsCount = 8;
...
class ThreadsMultipleTest : public ThreadsSingleTest {
  public:
    ...
    virtual void SetUp()
    {
        ...
        ASSERT_GT(s_multipleThreadsCount, 1U);
        m_multipleThreadsFinished = 0;
        for (uint32_t i = 0; i < s_multipleThreadsCount; i++) {
  ...
  private:
    ...
    static volatile uint32_t m_multipleThreadsFinished;
    ...
    static ThreadReturn JoinAnotherThreadTask(void* arg)
    {
        ...
        InterlockedIncrement(&m_multipleThreadsFinished);
        return nullptr;
    }
...
volatile uint32_t ThreadsMultipleTest::m_multipleThreadsFinished = 0;
...
{
    ASSERT_EQ(ER_OK, threadUnderTest->Start());
    ASSERT_NE(nullptr, threadUnderTest->GetHandle());

    ASSERT_EQ(ER_OK, threadUnderTest->Join());

    EXPECT_EQ(nullptr, threadUnderTest->GetHandle());
}
