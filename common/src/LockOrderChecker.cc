    ...
    : m_currentDepth(0), m_maximumDepth(defaultMaximumStackDepth)
{
    ...
    QCC_ASSERT(m_lockStack != nullptr);
}
...
void LockOrderChecker::AcquiringLock(const Mutex* lock)
{
    ...
    LockLevel previousLevel = m_lockStack[recentEntry].m_level;
    LockLevel lockLevel = lock->m_mutexInternal->GetLevel();
    ...
    if (lockLevel == LOCK_LEVEL_NOT_SPECIFIED) {
        if (enabledOptions & LOCKORDERCHECKER_OPTION_MISSING_LEVEL_ASSERT) {
            ...
            fprintf(stderr,
                    "Acquiring lock 0x%p with unspecified level (%s:%d). Current thread already owns lock 0x%p level %d (%s:%d).\n",
                    lock,
                    lock->m_mutexInternal->GetLatestOwnerFileName() ? lock->m_mutexInternal->GetLatestOwnerFileName() : "unknown file",
                    lock->m_mutexInternal->GetLatestOwnerLineNumber(),
                    previousTrace.m_lock,
                    previousTrace.m_level,
                    previousTrace.m_lock->m_mutexInternal->GetLatestOwnerFileName() ? previousTrace.m_lock->m_mutexInternal->GetLatestOwnerFileName() : "unknown file",
                    previousTrace.m_lock->m_mutexInternal->GetLatestOwnerLineNumber());
            fflush(stderr);
    ...
    if (!previouslyLocked && (enabledOptions & LOCKORDERCHECKER_OPTION_LOCK_ORDERING_ASSERT)) {
        LockTrace& previousTrace = m_lockStack[recentEntry];
        fprintf(stderr,
                "Acquiring lock 0x%p level %d (%s:%d). Current thread already owns lock 0x%p level %d (%s:%d).\n",
                lock,
                lockLevel,
                lock->m_mutexInternal->GetLatestOwnerFileName() ? lock->m_mutexInternal->GetLatestOwnerFileName() : "unknown file",
                lock->m_mutexInternal->GetLatestOwnerLineNumber(),
 ...
 */
void LockOrderChecker::LockAcquired(Mutex* lock)
{
    LockLevel lockLevel = lock->m_mutexInternal->GetLevel();
    QCC_ASSERT(lockLevel != LOCK_LEVEL_CHECKING_DISABLED);
...
void LockOrderChecker::ReleasingLock(const Mutex* lock)
{
    ...
    if (!previouslyLocked) {
        fprintf(stderr, "Current thread doesn't own lock 0x%p level %d.\n", lock, lockLevel);
        fflush(stderr);
