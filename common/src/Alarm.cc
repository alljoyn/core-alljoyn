...
_Alarm::_Alarm(Timespec<MonotonicTime> relativeTime, AlarmListener* listener, void* context, uint32_t periodMs)
    : alarmTime(relativeTime), listener(listener), periodMs(periodMs), context(context), id(IncrementAndFetch(&nextId))
{
}
...
_Alarm::_Alarm(AlarmListener* listener, void* context)
    : alarmTime(0), listener(listener), periodMs(0), context(context), id(IncrementAndFetch(&nextId))
{
}
