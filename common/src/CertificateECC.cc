...
static QStatus DecodeTime(uint64_t& epoch, const qcc::String& t)
{
    ...
    uint64_t localTime = qcc::ConvertStructureToTime(&tm);
    if (localTime < 0) {
        return ER_FAIL;
    }
    struct tm* gtm = qcc::ConvertTimeToStructure(&localTime);
    if (!gtm) {
        return ER_FAIL;
    }
    ...
    struct tm* ltm = qcc::ConvertToLocalTime(&localTime);
    if (!ltm) {
        return ER_FAIL;
    }
    if (ltm->tm_isdst > 0) {
...
{
    struct tm* ptm = qcc::ConvertTimeToStructure(&epoch);
    if (!ptm) {
        return ER_FAIL;
    }
    /**
    ...
    char buf[16];
    size_t ret;
    ret = qcc::FormatTime(buf, sizeof(buf), format, ptm);
    if (!ret) {
        return ER_FAIL;
    }
