                    ...
                    if (it->second.stopping_state == IO_RUNNING) {
                        if (&stream->GetSourceEvent() == *i) {
                            ...
                            if (it->second.writeEnable && !it->second.writeInProgress) {
                                ...
                                map<Stream*, IODispatchEntry>::iterator streamIt = dispatchEntries.find(stream);
                                while (isRunning && status == ER_TIMER_FULL &&  streamIt != dispatchEntries.end() && streamIt->second.stopping_state == IO_RUNNING) {
                                    /* Call the non-blocking version of AddAlarm, while holding the
                                     * locks to ensure that the state of the dispatchEntry is valid.
                                     */
                                    ...
                                    streamIt = dispatchEntries.find(stream);
                                }
                                if (status == ER_OK && streamIt != dispatchEntries.end()) {

                                    streamIt->second.writeAlarm = writeAlarm;
                                }
