...
bool IODispatch::IsRouterIdle(uint64_t minTime)
{
    /* The router node is considered idle if there are no connected leaf nodes
     * and no leaf node has disconnected during the minTime period.
     * Note that the router idle state can transition while this method is
