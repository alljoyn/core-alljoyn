...
void LockChecker::AcquiringLock(const Mutex* lock)
{
    ...
    if (lockLevel == LOCK_LEVEL_NOT_SPECIFIED) {
        if (enabledOptions & LOCKCHECKER_OPTION_MISSING_LEVEL_ASSERT) {
            ...
            QCC_LogError(ER_FAIL, ("Acquiring lock 0x%p with unspecified level (%s:%d). Current thread already owns lock 0x%p level %d (%s:%d).",
                                   lock, lock->file ? lock->file : "unknown file", lock->line,
                                   previousTrace.m_lock, previousTrace.m_level, previousTrace.m_lock->file ? previousTrace.m_lock->file : "unknown file", previousTrace.m_lock->line));
    ...
    if (!previouslyLocked && (enabledOptions & LOCKCHECKER_OPTION_LOCK_ORDERING_ASSERT)) {
        LockTrace& previousTrace = m_lockStack[recentEntry];
        QCC_LogError(ER_FAIL, ("Acquiring lock 0x%p level %d (%s:%d). Current thread already owns lock 0x%p level %d (%s:%d).",
                               lock, lockLevel, lock->file ? lock->file : "unknown file", lock->line,
                               previousTrace.m_lock, previousTrace.m_level, previousTrace.m_lock->file ? previousTrace.m_lock->file : "unknown file", previousTrace.m_lock->line));
...
void LockChecker::LockAcquired(Mutex* lock)
{
    ...
    for (uint32_t stackEntry = 0; stackEntry < m_currentDepth; stackEntry++) {
        ...
        if (m_lockStack[stackEntry].m_lock == lock) {
            ...
            if (newRecursionCount > lock->maximumRecursionCount) {
                lock->maximumRecursionCount = newRecursionCount;
