  ...
  private:
    class _TimerContext {
      ...
      private:
        ...
        DWORD debugReplaced;
        DWORD debugRemoved;
        DWORD debugCancelled;
        DWORD debugTORemoved;
        DWORD debugScheduled;
        DWORD debugFired;
    };
...
#endif
    volatile LONG debugWaitFailedSameThread;
    volatile LONG debugWaitTotal;
};

} /* namespace qcc */

static volatile LONG s_curTc = 1;
static DWORD GetTickCountt()
{
    LONG curTc = InterlockedIncrement(&s_curTc);
    return curTc;
}

TimerImpl::_TimerContext::_TimerContext() : active(true), ptpTimer(nullptr), threadId(0)
{
    debugCreated = GetTickCountt();
    debugAdded = 0;
    debugReplaced = 0;
    debugRemoved = 0;
    debugCancelled = 0;
...
QStatus TimerImpl::AddAlarm(const Alarm& alarm, bool canBlock)
{
    ...
    /* Duplicate alarms are okay and shouldn't have to wait */
    ...
    if (!duplicate) {
        for (auto timer : inFlightTimers) {
            if (timer->alarm == alarm) {
                duplicate = true;
                /* User wants this alarm added back, mark it as active */
                timer->active = true;
                break;
            }
        }
    ...
    if (!duplicate) {
        ...
        if (status == ER_OK) {
            ...
            if (sizeBefore == timers.size()) {
                ...
                newTimer->debugAdded = GetTickCountt();
                ...
                if (first->alarm == newTimer->alarm) {
                    ...
                    if (previous->ptpTimer != nullptr) {
                        ...
                        previous->debugCancelled = GetTickCountt();
...
void TimerImpl::OnTimeout(PTP_CALLBACK_INSTANCE instance, PTP_TIMER ptpTimerLocal)
{
    ...
    /* The first thing to do is to look for the active timer to trigger and take it out from the list */
    TimerContext trigger;
    bool releaseSemaphoreOnExit = false;
    Lock();
    if (!timers.empty()) {
        TimerContext first = *(timers.begin());
        if ((first->ptpTimer == ptpTimerLocal) && first->active) {
            /* Prepare to fire the alarm by moving it into the in-flight list */
            assert(first->ptpTimer != nullptr);
            trigger = first;
            trigger->threadId = GetCurrentThreadId();
            inFlightTimers.push_back(trigger);
            trigger->debugTORemoved = GetTickCountt();
            QCC_VERIFY(RemoveTimerByAlarm(trigger->alarm, false));
            releaseSemaphoreOnExit = true;
            assert(!inFlightTimers.empty());
            assert(inFlightTimers.back() == trigger);
        } else {
            /*
    ...
    /* Mark the triggered timer inactive except if it's periodic (active ones are moved back from the in-flight list) */
    if (trigger->ptpTimer != nullptr) {
    ...
    trigger->debugFired = GetTickCountt();
    if (IsRunning()) {
    ...
    /* If trigger is valid, expect the timer to have been removed from the in-flight list */
    assert((trigger->ptpTimer == nullptr) || removed);
    if (IsRunning() && removed) {
        closeTimerOnExit = false;
        /* Attempt to move active timer back into the timers list */
        if (trigger->active) {
...
QStatus TimerImpl::ReplaceAlarm(const Alarm& origAlarm, const Alarm& newAlarm, bool blockIfTriggered)
{
    ...
    Lock();
    if (running && !timers.empty()) {
        /* Cancel the timer if it's the first one */
        ...
        /* Remove the alarm, keeping the semaphore for the replacement alarm */
        found = RemoveTimerByAlarm(origAlarm, false);
        if (found) {
            /* Insert the replacement alarm */
            TimerContext newTimer;
            newTimer->alarm = newAlarm;
            newTimer->ptpTimer = ptpTimerLocal;
            size_t sizeBefore = timers.size();
            timers.insert(newTimer);
            if (sizeBefore == timers.size()) {
                /* Failed to insert (newAlarm is dup), release the semaphore since we've already removed origAlarm */
                assert(--debugSemaphore >= 0);
                QCC_VERIFY(ReleaseSemaphore(alarmSemaphore, 1, nullptr));
            } else {
                newTimer->debugReplaced = GetTickCountt();
                /* The new timer added successfully */
                ptpTimerLocal = nullptr;
            }
            /* Schedule the first timer if needed */
            TimerContext first = *(timers.begin());
            if (cancelled || (first->alarm == newTimer->alarm)) {
#ifndef NDEBUG
    ...
    if (wait != nullptr) {
        InterlockedIncrement(&debugWaitTotal);
        if (!IsTimerCallbackThread()) {
            ...
            LONG sameThread = InterlockedIncrement(&debugWaitFailedSameThread);
            printf("\r\n!!! sameThread1 count=%d.\r\n", sameThread);
        }
    }
    return ER_OK;
...
bool TimerImpl::RemoveAlarm(const Alarm& alarm, bool blockIfTriggered)
{
    ...
    if (running && !timers.empty()) {
        ...
        if (first->alarm == alarm) {
            ...
            assert(first->ptpTimer != nullptr);
            first->active = false;
            first->debugCancelled = GetTickCountt();
            first->debugRemoved = GetTickCountt();
            SetThreadpoolTimer(first->ptpTimer, nullptr, 0, 0);
        ...
        if (!found) {
            ...
            for (auto timer : timers) {
                if (timer->alarm == alarm) {
                    timer->debugRemoved = GetTickCountt();
                    break;
                }
            }
#endif
