...
class SuperWaiter {
  public:
    ...
    QStatus WaitForMultipleObjects(_In_ uint32_t totalHandles, _In_reads_(totalHandles) const HANDLE* handles, _In_ uint32_t timeoutMsec, _Out_ uint32_t* indexSignaled)
    {
        ...
        m_signaled = false;
        *indexSignaled = m_indexSignaled = 0xFFFFFFFF;
  ...
  private:
    ...
    static VOID CALLBACK s_WaitThread(_Inout_ PTP_CALLBACK_INSTANCE instance, _Inout_opt_ PVOID context, _Inout_ PTP_WORK work)
    {
        ...
        /*
           printf("Instance %p : waiting for %u handles:\n", instance, numHandles);
           for (uint32_t currentHandle = 0; currentHandle < numHandles; currentHandle += 1) {
            printf("%p ", handles[currentHandle]);
           }
           printf("\n");
         */
...
QStatus Event::Wait(const vector<Event*>& checkEvents, vector<Event*>& signaledEvents, uint32_t maxWaitMs)
{
    ...
    for (it = checkEvents.begin(); it != checkEvents.end(); ++it) {
        ...
        if (evt->timerHandle != NULL) {
            // printf("Will wait for timer %p , timeout %u\n", evt->timerHandle, maxWaitMs);
        ...
        for (it = checkEvents.begin(); it != checkEvents.end(); ++it) {
            ...
            if (somethingSet) {
                if (evt->eventType == TIMED) {
                     ...
                     * switched this timer to non-signaled state, so IsSet() would return incorrectly false.
                     */
                    if (evt->timerHandle != nullptr && evt->timerHandle == signaledHandle) {
                        // printf("Timer %p was signaled\n", evt->timerHandle);
    ...
    isSocket(false)
{
    ...
    if (timerHandle != NULL && delay != WAIT_FOREVER) {
        ...
        // printf("SetWaitableTimer %p , delay = %I64d , period = %u\n", this->timerHandle, timerDelay.QuadPart, (ULONG) this->period);
