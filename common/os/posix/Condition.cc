...
QStatus Condition::Wait(qcc::Mutex& m)
{
    ...
    int ret = pthread_cond_wait(&c, m_mutexInternal->GetPlatformSpecificMutex());
...
QStatus Condition::TimedWait(qcc::Mutex& m, uint32_t ms)
{
    ...
    m.m_mutexInternal->ReleasingLock();
    int ret = pthread_cond_timedwait(&c, m_mutexInternal->GetPlatformSpecificMutex(), &tsTimeout);
    m.m_mutexInternal->LockAcquired();
