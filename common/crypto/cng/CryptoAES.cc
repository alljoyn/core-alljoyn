...
QStatus Crypto_AES::Encrypt(const Block* in, Block* out, uint32_t numBlocks)
{
    ...
    ULONG clen;
    if (!BCRYPT_SUCCESS(BCryptEncrypt(keyState->handle, static_cast<PUCHAR>(&in->data[0]), len, NULL, NULL, 0, static_cast<PUCHAR>(&out->data[0]), len, &clen, 0))) {
        return ER_CRYPTO_ERROR;
    } else {
        return ER_OK;
    }
...
QStatus Crypto_AES::Encrypt_CCM(const void* in, void* out, size_t& len, const KeyBlob& nonce, const void* addData, size_t addLen, uint8_t authLen)
{
    ...
    ULONG clen;
    Block iv;
    NTSTATUS ntstatus = BCryptEncrypt(keyState->handle, static_cast<PUCHAR>(in), len, &cmi, NULL, 0, static_cast<PUCHAR>(out), len, &clen, 0);
