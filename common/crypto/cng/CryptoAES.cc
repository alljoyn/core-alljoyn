...
QStatus Crypto_AES::Encrypt(const Block* in, Block* out, uint32_t numBlocks)
{
    ...
    ULONG clen;
    if (!BCRYPT_SUCCESS(BCryptEncrypt(keyState->handle, (PUCHAR)in.data, len, NULL, NULL, 0, (PUCHAR)out.data, len, &clen, 0))) {
        return ER_CRYPTO_ERROR;
    } else {
        return ER_OK;
    }
...
QStatus Crypto_AES::Encrypt_CCM(const void* in, void* out, size_t& len, const KeyBlob& nonce, const void* addData, size_t addLen, uint8_t authLen)
{
    ...
    ULONG clen;
    Block iv;
    NTSTATUS ntstatus = BCryptEncrypt(keyState->handle, (PUCHAR)in.data, len, &cmi, NULL, 0, (PUCHAR)out.data, len, &clen, 0);
