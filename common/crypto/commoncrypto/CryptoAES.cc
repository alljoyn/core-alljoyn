...
static void Compute_CCM_AuthField(KeyBlob* key, Crypto_AES::Block& T, uint8_t M, uint8_t L, const KeyBlob& nonce, const uint8_t* mData, size_t mLen, const uint8_t* addData, size_t addLen)
{
    size_t dataMoved;
    ...
    CCCryptorCreateWithMode(kCCEncrypt, kCCModeCBC, kCCAlgorithmAES128, ccNoPadding,
                            ivec.data,  key->GetData(), key->GetSize(), NULL, 0, 0, 0, &cryptor);
    Trace("CBC IV in: ", B_0.data, sizeof(B_0.data));
    CCCryptorUpdate(cryptor, B_0.data, kCCBlockSizeAES128, T.data, kCCBlockSizeAES128, &dataMoved);
    ...
    if (addLen) {
         ...
         */
        CCCryptorUpdate(cryptor, A.data, kCCBlockSizeAES128, T.data, kCCBlockSizeAES128, &dataMoved);
        Trace("After AES: ", T.data, sizeof(T.data));
        while (addLen >= sizeof(Crypto_AES::Block)) {
            CCCryptorUpdate(cryptor, addData, kCCBlockSizeAES128, T.data, kCCBlockSizeAES128, &dataMoved)
            Trace("After AES: ", T.data, sizeof(T.data));
    ...
    if (mLen) {
        while (mLen >= sizeof(Crypto_AES::Block)) {
            CCCryptorUpdate(cryptor, mData, kCCBlockSizeAES128, T.data, kCCBlockSizeAES128, &dataMoved)
            Trace("After AES: ", T.data, sizeof(T.data));
        ...
        if (mLen) {
            ...
            final.Pad(16 - mLen);
            CCCryptorUpdate(cryptor, final.data, kCCBlockSizeAES128, T.data, kCCBlockSizeAES128, &dataMoved)
            Trace("After AES: ", T.data, sizeof(T.data));
        }
    }
    CCCryptorFinal(cryptor, T.data, kCCBlockSizeAES128, &dataMoved);
...
QStatus Crypto_AES::Encrypt_CCM(const void* in, void* out, size_t& len, const KeyBlob& nonce, const void* addData, size_t addLen, uint8_t authLen)
{
    ...
    CCCryptorRef cryptor;
    size_t dataMoved;
    CCCryptorStatus status = CCCryptorCreateWithMode(kCCEncrypt, kCCModeCTR, kCCAlgorithmAES128, ccNoPadding,
     ...
     */
    Block U;
    status = CCCryptorUpdate(cryptor, (const unsigned char*)T.data, kCCBlockSizeAES128, U.data, kCCBlockSizeAES128, &dataMoved);
    ...
    status = CCCryptorUpdate(cryptor, (const unsigned char*)in, len, out, len, &dataMoved);
    if (status != kCCSuccess) {
        CCCryptorRelease(cryptor);
        return ER_CRYPTO_ERROR;
    }
    CCCryptorFinal(cryptor, out, len, &dataMoved);
    CCCryptorRelease(cryptor);
...
QStatus Crypto_AES::Decrypt_CCM(const void* in, void* out, size_t& len, const KeyBlob& nonce, const void* addData, size_t addLen, uint8_t authLen)
{
    ...
    status = CCCryptorUpdate(cryptor, (const unsigned char*)U.data, sizeof(T.data), T.data, sizeof(T.data), &dataMoved);
    if (status != kCCSuccess) {
        CCCryptorRelease(cryptor);
        return ER_AUTH_FAIL;
    }
