...
QStatus Crypto_Hash::Init(Algorithm alg, const uint8_t* hmacKey, size_t keyLen)
{
    ...
    case qcc::Crypto_Hash::SHA1:
        ...
        if (MAC) {
            ...
            /*
             * CC_SHA* functions always return 1 by documentation
             */
            QCC_VERIFY(1 == CC_SHA1_Init(&ctx->md.sha1));
        }
        initialized = true;
        break;
    ...
    case qcc::Crypto_Hash::SHA256:
        ...
        if (MAC) {
            ...
            QCC_VERIFY(1 == CC_SHA256_Init(&ctx->md.sha256));
        }
        initialized = true;
        break;
...
QStatus Crypto_Hash::Update(const uint8_t* buf, size_t bufSize)
{
    ...
    if (initialized) {
        if (MAC) {
            ...
            QCC_VERIFY(1 == CC_SHA1_Update(&ctx->md.sha1, buf, bufSize));
        } else if (ctx->algorithm == qcc::Crypto_Hash::SHA256) {
            QCC_VERIFY(1 == CC_SHA256_Update(&ctx->md.sha256, buf, bufSize));
        } else {
            status = ER_CRYPTO_ERROR;
...
{
    return Update(reinterpret_cast<const uint8_t*>(str.data()), str.size());
}
    ...
    if (initialized) {
        if (MAC) {
            ...
            if (ctx->algorithm == qcc::Crypto_Hash::SHA1) {
                QCC_VERIFY(1 == CC_SHA1_Final(digest, &ctx->md.sha1));
            } else if (ctx->algorithm == qcc::Crypto_Hash::SHA256) {
                QCC_VERIFY(1 == CC_SHA256_Final(digest, &ctx->md.sha256));
            } else {
                status = ER_CRYPTO_ERROR;
