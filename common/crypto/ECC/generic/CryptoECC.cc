...
QStatus ECCPublicKey::Import(const uint8_t* data, size_t size)
{
    ...
    /* Verify that this public key is a point on the curve. */
    affine_point_t pub;
    const size_t coordinateSize = GetCoordinateSize();

    pub.infinity = 0;
    binary_to_bigval(data, &pub.x, coordinateSize);
    binary_to_bigval(data + coordinateSize, &pub.y, coordinateSize);
