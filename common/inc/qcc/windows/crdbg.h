 ...
 * from making automatic memory dumps in Jenkins for Windows builds. This version
...
#define QCC_ASSERT(expr) (void) (                                                                    \
        (!!(expr)) ||                                                                                \
        (0 == printf("%s(%d) : Assertion failed for expression: %s.", __FILE__, __LINE__, #expr)) || \
        (*((volatile int*)0) == 0)                                                                   \
    )

#define QCC_CRASHING_ASSERT