...
bool _AboutObjHost::announce(const NPVariant* args, uint32_t argCount, NPVariant* result) {
    ...
    if (NPN_GetProperty(plugin->npp, NPVARIANT_TO_OBJECT(args[1]),
        ...
        case NPVariantType_String: {
                ...
                if (!NPVARIANT_IS_VOID(variant)) {
                    qcc::String language = ToDOMString(plugin, variant, typeError);
                    int32_t utf8length = language.length();
                    SupportedLanguages[0] = new char[utf8length + 1];
                    strncpy(SupportedLanguages[0], language.c_str(), utf8length);
                    SupportedLanguages[0][utf8length] = 0;
                }
                break;
            }
