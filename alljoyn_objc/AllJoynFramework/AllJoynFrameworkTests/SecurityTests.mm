...
static NSString *const kCorrectPsk = @"1234";
static NSString *const kIncorrectPsk = @"4321";

static NSString *const kSpekePassword = @"ABCD";

static NSString *const kSecureObjectPath = @"/SecureObject";

static NSString *const kSecureServiceName = @"org.alljoyn.unittests";
...
@property (nonatomic, nonnull, strong) AJNGUID128 *managerGuid;
...
- (void)testSecureMethodCallEcdhePsk
{
    ...
    XCTAssertTrue(client.authenticationState == kAuthSuccess, @"Auth on client side was failed.");
...
- (void)testSecureMethodCallEcdhePskWrongClientPsk
{
    [self alljoynServiceOn];
...
- (void)testSecureMethodCallEcdheSpeke
{
    [self alljoynServiceOn];
    ...
    //enable peer security
    status = [self.bus enablePeerSecurity:@"ALLJOYN_ECDHE_SPEKE" authenticationListener:self keystoreFileName:[NSString stringWithFormat:@"%@/%@", currentDir, @"serviceBusKeystore"] sharing:YES];
    XCTAssertTrue(status == ER_OK, @"Service bus failed to enable peer security.");
    [self.bus clearKeyStore];

    status = [client.bus enablePeerSecurity:@"ALLJOYN_ECDHE_SPEKE" authenticationListener:client keystoreFileName:[NSString stringWithFormat:@"%@/%@", currentDir, @"clientBusKeystore"] sharing:YES];
    XCTAssertTrue(status == ER_OK, @"Client bus failed to enable peer security.");
    [client.bus clearKeyStore];
    ...
    XCTAssertTrue(self.authenticationState == kAuthSuccess, @"Auth on service side was failed.");
    XCTAssertTrue(client.authenticationState == kAuthSuccess, @"Auth on сlient side was failed.");
...
- (void)testSecureMethodCallWithSecurityManager
{
    ...
    XCTAssertTrue(managerSessionId > 0, @"ManagerBus join session on port %ld failed.", (long)kSessionDefaultPort);
    ...
    XCTAssertTrue(serviceSessionId > 0, @"ManagerBus join session on port %ld failed.", (long)kSessionDefaultPort);
    ...
    XCTAssertTrue(clientSessionId > 0, @"ManagerBus join session on port %ld failed.", (long)kSessionDefaultPort);
        ...
        if (mask & kAJNSecurityCredentialTypePassword) {
            if (authenticationCount <= 3) {
                ...
                if (!self.isWrongPasswordMustBeUsed) {
                    credentials.password = kCorrectPsk;
                } else {
                    credentials.password = kIncorrectPsk;
                }
            }
        }
    }
    else if ([authenticationMechanism compare:@"ALLJOYN_ECDHE_SPEKE"] == NSOrderedSame) {
        if (mask & kAJNSecurityCredentialTypePassword) {
            if (authenticationCount <= 3) {
                credentials = [[AJNSecurityCredentials alloc] init];
                credentials.password = kSpekePassword;
            }
        }
        if (mask & kAJNSecurityCredentialTypeExpirationTime) {
            credentials = [[AJNSecurityCredentials alloc] init];
            credentials.expirationTime = [[NSDate date] timeIntervalSince1970] + 3600;
        }
    }
