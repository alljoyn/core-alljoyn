...
@interface AJNManifest (Private)

@property (nonatomic, readonly) Manifest *manifest;
...
- (QStatus)getPolicy:(AJNPermissionPolicy *)policy
{
    ...
    return self.securityAppProxy->GetPolicy(*policy.permissionPolicy);
}

- (QStatus)getDefaultPolicy:(AJNPermissionPolicy *)defaultPolicy
{
    if (defaultPolicy == nil) {
        return ER_BAD_ARG_1;
    }

    return self.securityAppProxy->GetDefaultPolicy(*defaultPolicy.permissionPolicy);
}

- (QStatus)claim:(AJNKeyInfoNISTP256*)certificateAuthority adminGroupId:(AJNGUID128*)adminGroupId adminGroup:(AJNKeyInfoNISTP256*)adminGroup identityCertChain:(NSArray*)identityCertChain manifests:(NSArray*)manifests
{
    size_t certCount = identityCertChain.count;
    size_t manifestCount = manifests.count;

    qcc::CertificateX509 *certs = new qcc::CertificateX509[certCount];
    Manifest *manifestList = new Manifest[manifestCount];
    ...
    for (int i = 0; i < manifestCount; i++) {
        AJNManifest *manifest = manifests[i];
        manifestList[i] = *manifest.manifest;
    }
...
{
    if (policy != nil) {
        return self.securityAppProxy->UpdatePolicy(*policy.permissionPolicy);
    }
...
+ (void)destroySignedManifest:(char*)signedManifestXml
{
    ...
    return ajn::SecurityApplicationProxy::DestroySignedManifest(signedManifestXml);
}
...
+ (QStatus)computeManifestDigest:(NSString*)unsignedManifestXml identityCertificate:(AJNCertificateX509*)identityCertificate digest:(NSMutableData*)digest
{
    ...
    if (outDigest != NULL) {
        NSData *tmpData = [NSData dataWithBytes:outDigest length:outDigestSize];
        [digest setData:tmpData];
    }
