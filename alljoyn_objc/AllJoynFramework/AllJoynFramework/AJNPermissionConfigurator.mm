...
- (QStatus)getSigningPublicKey:(AJNKeyInfoNISTP256 **)keyInfo
{
    ...
    qcc::KeyInfoNISTP256 *qccKeyInfo = new qcc::KeyInfoNISTP256();
    QStatus status = self.permissionConfigurator->GetSigningPublicKey(*qccKeyInfo);
    
    if (status == ER_OK) {
        *keyInfo = [[AJNKeyInfoNISTP256 alloc] initWithHandle:(AJNHandle)qccKeyInfo shouldDeleteHandleOnDealloc:YES];
    }
...
- (QStatus)getConnectedPeerPublicKeyForUid:(AJNGUID128 *)guid publicKey:(AJNECCPublicKey **)publicKey
{
    ...
    qcc::ECCPublicKey *qccPublicKey = new qcc::ECCPublicKey();
    QStatus status = self.permissionConfigurator->GetConnectedPeerPublicKey(*guid.guid128, qccPublicKey);
    
    if (status == ER_OK) {
        *publicKey = [[AJNECCPublicKey alloc] initWithHandle:(AJNHandle)qccPublicKey shouldDeleteHandleOnDealloc:YES];
    }
