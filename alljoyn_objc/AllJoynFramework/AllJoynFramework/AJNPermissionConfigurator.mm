...
{
    std::vector<Manifest> cert;
    QStatus status = self.permissionConfigurator->GetManifests(cert);
    for (auto &it : cert) {
        [manifests addObject:[[AJNManifest alloc] initWithHandle:(AJNHandle)&it]];
    }
...
{
    return self.permissionConfigurator->UpdatePolicy(*policy.permissionPolicy);
}

- (QStatus)getPolicy:(AJNPermissionPolicy*)policy
{
    return self.permissionConfigurator->GetPolicy(*policy.permissionPolicy);
}

- (QStatus)getDefaultPolicy:(AJNPermissionPolicy*)policy
{
    return self.permissionConfigurator->GetDefaultPolicy(*policy.permissionPolicy);
}
