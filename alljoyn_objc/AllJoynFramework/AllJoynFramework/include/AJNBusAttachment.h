 ...
 * it is activated.
 *
 * @param[in] interfaceName The requested interface name.
 * @param[in] interfaceSecurityPolicy The security policy for this interface
 *
 * @return Interface description, or nil if cannot be created
 */
 ...
 *
 * @param busObject BusObject to register.
 * @param shouldEnableSecurity The true if authentication is required to access this object.
 *
 * @return
 *      - ER_OK if successful.
 *      - ER_BUS_BAD_OBJ_PATH for a bad object path
 ...
 *          - Other error status codes indicating a failure
 */
- (QStatus)disconnectWithArguments:(NSString *)connectionArguments DEPRECATED_ATTRIBUTE;
 ...
 * @param[in]  leaveSessionDelegate Called when LeaveSession response is received.
 ...
 *
 * @param[in]  sessionId     Session id.
 * @param[in]  leaveSessionDelegate      Called when LeaveJoinedSession response is received.
 * @param[in]  context       User defined context which will be passed as-is to callback.
 *
 * @return
 *      - ER_OK iff method call to local router response was successful.
 ...
 *
 * @param[in]  sessionId Session id.
 * @param[in]  leaveSessionDelegate Called when LeaveHostedSession response is received.
 * @param[in]  context User defined context which will be passed as-is to callback.
 *
 * @return
 *      - ER_OK iff method call to local router response was successful.
 ...
 *
 * @param[in]  rule Match rule to be added (see DBus specification for format of this string).
 * @param[in]  addMatchDelegate The object to be called when AddMatchAsync completes.
 ...
 * @param[in]  removeMatchDelegate The object to be called when RemoveMatchAsync completes.
 * @param[in]  context  User-defined context that will be returned to the reply handler
 *
 * @return
 ...
 * @param permissionConfigListener   Passes security 2.0 callbacks to the application.
 *
 ...
 * @param[in] getNameOwnerDelegate pointer to function that is called with the results
 ...
 *
 * @param[in] interfaces a list of interfaces that the Announce
 ...
 * @see WhoImplements(const char**, size_t)
 *
 * @param[in] interface     interface that the remote user must implement to
 *                          receive the announce signal.
 * @return
 *    - ER_OK on success
 *    - An error status otherwise
 ...
 * @param[in] interfaces a list of interfaces. The list must match the
 ...
 * Non-blocking variant of cancelWhoImplements.
 *
 * @param[in] interface a list of interfaces. The list must match the
 *                                 list previously passed to the WhoImplements
 *                                 member function
 * @return
 *    - ER_OK on success
 ...
 * org.alljoyn.Bus.Application ыtate signal.
 *
