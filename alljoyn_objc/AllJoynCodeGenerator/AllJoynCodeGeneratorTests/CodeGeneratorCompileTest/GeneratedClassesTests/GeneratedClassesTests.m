...
- (void)testCallGeneratedMethodsShouldBeSuccess {
    ...
    XCTAssertTrue([outString1 isEqualToString:@"foobar"], @"Enexpected response from proxy object");
    ...
    XCTAssertTrue([outString1 isEqualToString:@"foo+1"]);
    XCTAssertTrue([outString2 isEqualToString:@"foo+1"]);
    ...
    XCTAssertTrue([outString1 isEqualToString:@"foo+1"]);
    XCTAssertTrue([outString2 isEqualToString:@"bar+1"]);

    status = [_basicProxyObject methodWithOnlyOutString:&outString1 outString2:&outString2];
    XCTAssertTrue(status == ER_OK, @"Failed to call methodWithOnlyOutString.");
    XCTAssertTrue([outString1 isEqualToString:@"1st string"]);
    XCTAssertTrue([outString2 isEqualToString:@"2st string"]);
    ...
    XCTAssertTrue([outString1 isEqualToString:@"only out string"]);

    //TODO: test methodWithStringArray
}

- (void)testCallGeneratedMethodsShouldBeFailerAfterDestroyBusObject
{
    _basicObject = nil;
    QStatus status = ER_OK;
