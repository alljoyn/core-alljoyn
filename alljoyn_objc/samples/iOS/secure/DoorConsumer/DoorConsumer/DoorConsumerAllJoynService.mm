...
- (BOOL)openDoors
{
    if (_doorProxyMutableArray.count == 0) {
        [_messageListener didReceiveAllJoynStatusMessage: @"There are no any doors to open\n"];
        return NO;
    }
    for (DoorProxy *proxy in _doorProxyMutableArray) {
        BOOL success;
        if ([proxy open: &success] == ER_OK) {
            if (success == YES) {
                [_messageListener didReceiveAllJoynStatusMessage: [NSString stringWithFormat:@"Door %@ is opened successfully\n", proxy]];
            } else {
                [_messageListener didReceiveAllJoynStatusMessage: [NSString stringWithFormat:@"Opening door %@ is failed\n", proxy]];
            }
        } else {
            [_messageListener didReceiveAllJoynStatusMessage: @"Calling open door method is failed\n"];
        }
    }
    return YES;
};
...
{
    if (_doorProxyMutableArray.count == 0) {
        [_messageListener didReceiveAllJoynStatusMessage: @"There are no any doors to close\n"];
        return NO;
    }
    for (DoorProxy *proxy in _doorProxyMutableArray) {
        BOOL success;
        if ([proxy close: &success] == ER_OK) {
            if (success == YES) {
                [_messageListener didReceiveAllJoynStatusMessage: [NSString stringWithFormat:@"Door %@ is closed successfully\n", proxy]];
            } else {
                [_messageListener didReceiveAllJoynStatusMessage: [NSString stringWithFormat:@"Closing door %@ is failed\n", proxy]];
            }
        } else {
            [_messageListener didReceiveAllJoynStatusMessage: @"Calling close door method is failed\n"];
        }
    }
    return YES;
}
...
{
    if (_doorProxyMutableArray.count == 0) {
        [_messageListener didReceiveAllJoynStatusMessage: @"There are no any doors to check the state\n"];
        return NO;
    }
    for (DoorProxy *proxy in _doorProxyMutableArray) {
        BOOL state;
        if ([proxy getStateMethod: &state] == ER_OK) {
            [_messageListener didReceiveAllJoynStatusMessage: [NSString stringWithFormat:@"Get door state is %s", state ? "opened\n" : "closed\n"]];
        } else {
            [_messageListener didReceiveAllJoynStatusMessage: @"Calling getState method is failed\n"];
        }
    }
    return YES;
}
...
{
    if (_doorProxyMutableArray.count == 0) {
        [_messageListener didReceiveAllJoynStatusMessage: @"There are no any doors to check the state property\n"];
        return NO;
    }
    for (DoorProxy *proxy in _doorProxyMutableArray) {
        BOOL state;
        if ([proxy getState: &state] == ER_OK) {
            [_messageListener didReceiveAllJoynStatusMessage: [NSString stringWithFormat:@"Get door state property is %s", state ? "opened\n" : "closed\n"]];
        } else {
            [_messageListener didReceiveAllJoynStatusMessage: @"Calling getState property is failed\n"];
        }
    }
    return YES;
}
...
- (void)foundDoorObjectAt:(NSString *)busName port:(AJNSessionPort)port;
{
    ...
    [_busAttachment joinSessionAsyncWithName:busName onPort:port withDelegate:self options:sessionOptions joinCompletedBlock:^(QStatus status, AJNSessionId sessionId, AJNSessionOptions *opts, void *context) {
        if (status == ER_OK) {
            ...
            [_messageListener didReceiveAllJoynStatusMessage:[NSString stringWithFormat:@"Door has been found at: %@.\n", busName]];
        }
        else {
            [_messageListener didReceiveAllJoynStatusMessage:[NSString stringWithFormat:@"Join session failed to connect on port %d\n", port]];
        }
    } context:nil];
}
...
- (QStatus)run
{
    ...
    [_messageListener didReceiveAllJoynStatusMessage:@"Door consumer was claimed.\n"];
