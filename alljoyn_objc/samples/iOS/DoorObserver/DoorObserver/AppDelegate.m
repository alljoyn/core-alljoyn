...
- (void)initAbout {
    ...
    status = [self.aboutData setAppId:appId];
    status = [self.aboutData setDeviceName:@"Device Name" andLanguage:@"en"];
    //DeviceId is a string encoded 128bit UUID
    status = [self.aboutData setDeviceId:@"avec-awe1213-1234559xvc123"];
    status = [self.aboutData setAppName:@"App Name" andLanguage:@"en"];
    status = [self.aboutData setManufacturer:@"Manufacturer" andLanguage:@"en"];
    status = [self.aboutData setModelNumber:@"ModelNo"];
    status = [self.aboutData setDescription:@"A poetic description of this application" andLanguage:@"en"];
    status = [self.aboutData setDateOfManufacture:@"DateOfManufacture"];
    status = [self.aboutData setSoftwareVersion:@"1.0"];
    status = [self.aboutData setHardwareVersion:@"16.10.00"];
    status = [self.aboutData setSupportUrl:@"some.random.url"];
...
- (QStatus)getAboutDataForLanguage:(NSString *)language usingDictionary:(AJNAboutData **)aboutData
{
    return [self getDefaultAnnounceData:aboutData];
}

-(QStatus)getDefaultAnnounceData:(AJNAboutData **)aboutData
{
...
- (QStatus)getAboutData:(AJNMessageArgument**)msgArg withLanguage:(NSString*)language
{
    ...
    *msgArg = [[AJNMessageArgument alloc] init];
    return status;
}
