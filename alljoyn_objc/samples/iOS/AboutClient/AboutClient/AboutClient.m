...
@interface AboutClient() <AJNBusListener, AJNJoinSessionDelegate, AJNSessionListener, AJNAboutListener>

@property (nonatomic, strong) NSCondition *receivedAnnounce;
@property (nonatomic, strong) AJNBusAttachment *bus;
@property (nonatomic, strong) NSString *foundServiceName;
@property (nonatomic, strong) AJNMessageArgument *aboutDataArg;
...
- (void)didReceiveAnnounceOnBus:(NSString *)busName withVersion:(uint16_t)version withSessionPort:(AJNSessionPort)port withObjectDescription:(AJNMessageArgument *)objectDescriptionArg withAboutDataArg:(AJNMessageArgument *)aboutDataArg
{
    ...
    QStatus status = [self.bus joinSessionAsyncWithName:busName onPort:port withDelegate:self options:sessionOptions joinCompletedDelegate:self context:nil];
    NSLog(@"joinSessionAsyncWithName status : %u", status);
}
...
#pragma mark - AJNJoinSessionDelegate method
...
{
    if (status == ER_OK) {
        [self.delegate didReceiveStatusUpdateMessage:[NSString stringWithFormat:@"Successfully joined session with id %d", sessionId]];

        // Create AboutProxy
        AJNAboutProxy *aboutProxy = [[AJNAboutProxy alloc] initWithBusAttachment:self.bus busName:_foundServiceName sessionId:sessionId];

        // Make a call to GetAboutData and GetVersion
        uint16_t aboutVersion;
        NSMutableDictionary *aboutDataDictionary;
        [aboutProxy getVersion:&aboutVersion];
        [aboutProxy getAboutDataForLanguage:@"en" usingDictionary:&aboutDataDictionary];
        NSLog(@"About data xml: %@", _aboutDataArg.xml);
        [self.delegate didReceiveStatusUpdateMessage:[NSString stringWithFormat:@"About proxy version is %d", _version]];
        AJNMessageArgument *modelNumberArgument = aboutDataDictionary[@"ModelNumber"];
        NSString *modelNumber = [[modelNumberArgument.xml stringByReplacingOccurrencesOfString:@"<string>" withString:@""] stringByReplacingOccurrencesOfString:@"</string>" withString:@""];
        [self.delegate didReceiveStatusUpdateMessage:[NSString stringWithFormat:@"Model number from about proxy data is %@", modelNumber]];
    }
    else {
        [self.delegate didReceiveStatusUpdateMessage:@"JoinSession failed to connect asynchronously"];
    }
