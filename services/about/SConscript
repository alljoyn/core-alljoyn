...
# Make about library and header file paths available to the global environment
...
for b in about_env['bindings']:
    if os.path.exists('%s/SConscript' % b):
        about_env.VariantDir('$OBJDIR/%s' % b, b, duplicate = 0)

about_env.SConscript(['$OBJDIR/%s/SConscript' % b for b in env['bindings'] if os.path.exists('%s/SConscript' % b) ],
                     exports = ['about_env'])
