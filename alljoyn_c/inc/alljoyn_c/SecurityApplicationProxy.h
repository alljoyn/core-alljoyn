 ...
 *                                      using the correct deallocating API.
 *
 * @return
 *          - #ER_OK If successful.
 *          - Other error status codes indicating a failure.
 ...
 *
 * @param[in]    proxy                  The alljoyn_securityapplicationproxy connected to the managed application.
 * @param[out]   digest                 A byte array containing the digest
 * @param[in]    expectedSize           expected length of the byte array
 *
 * @return
 *          - #ER_OK If successful.
 *          - Other error status codes indicating a failure.
 ...
 *
 * @param[in]   proxy   The alljoyn_securityapplicationproxy connected to the managed application.
 * @param[in]    serial  Null-terminated C String representing the serial number
 * @param[in]   pemKey  Null-terminated C String representing the public key of the certificate to be removed in PEM format
 *
 * @return
 *          - #ER_OK if successful
 *          - #ER_PERMISSION_DENIED Error raised when the caller does not have permission
 ...
 *          - An error status indicating failure.
 */
AJ_API QStatus AJ_CALL alljoyn_securityapplicationproxy_removemembership(alljoyn_securityapplicationproxy proxy, AJ_PCSTR serial, AJ_PCSTR pubKey);

/**
 * Get the summaries for installed memberships
 *
 * @param[in]   proxy           The alljoyn_securityapplicationproxy connected to the managed application.
 * @param[out]  certificateIds  Array of alljoyn_certificateid
 *
 * @return
 *  - #ER_OK if successful
 *  - an error status indicating failure
...
/**
 * Install signed manifests to application itself. This method only verifies that manifests have a signature;
 * it does not verify that the signature is valid. Only supports append mode
 *
