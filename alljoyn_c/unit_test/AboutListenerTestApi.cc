...
static void
about_test_wildcard_about_listener_announced_cb(const void* context,
                                                const char* busName,
                                                uint16_t version,
                                                alljoyn_sessionport port,
                                                const alljoyn_msgarg objectDescriptionArg,
                                                           ...
                                                           const alljoyn_msgarg aboutDataArg)
{
    ...
    alljoyn_msgarg_stabilize(aboutDataArg);
    listener->aboutData = alljoyn_aboutdata_create_full(aboutDataArg, "en");
}

static void
remove_object_description_about_listener_cb(const void* context,
                                            const char* busName,
                                            uint16_t version,
                                            alljoyn_sessionport port,
                                            const alljoyn_msgarg objectDescriptionArg,
...
void destroy_announce_non_128_bit_app_id_about_listener(announce_non_128_bit_app_id_about_listener* aboutListener)
{
    if (aboutListener) {
        ...
        if (aboutListener->aboutData) {
            alljoyn_aboutdata_destroy(aboutListener->aboutData);
