...
class SecurityApplicationProxyFullSetupTest : public SecurityApplicationProxyPostClaimTest {
  ...
  private:
    ...
    void InstallMembershipToManagedApp()
    {
        m_certIds.count = 0;
        EXPECT_EQ(ER_OK, alljoyn_securityapplicationproxy_installmembership(m_managedAppSecurityApplicationProxy, m_managedAppIdentityCertificate));
    }
};
...
{
    EXPECT_EQ(ER_XML_INVALID_ELEMENT_CHILDREN_COUNT, alljoyn_securityapplicationproxy_signmanifest(s_invalidManifest,
                                                                                                   m_securityManagerIdentityCertificate,
                                                                                                   m_securityManagerPrivateKey,
                                                                                                   &m_signedManifestXml));
}
...
TEST_F(SecurityApplicationProxyFullSetupTest, shouldPassGetIdentityManagedAppProxy)
{
    ...
    EXPECT_EQ(ER_OK, alljoyn_securityapplicationproxy_getidentity(m_managedAppSecurityApplicationProxy, &m_managedAppIdentityCertificateChain, &size));
    EXPECT_EQ(2U, size);
}
...
TEST_F(SecurityApplicationProxyFullSetupTest, shouldPassGetIdentitySecurityManagerProxy)
{
    ...
    EXPECT_EQ(ER_OK, alljoyn_securityapplicationproxy_getidentity(m_securityManagerSecurityApplicationProxy, &m_managedAppIdentityCertificateChain, &size));
    EXPECT_EQ(1U, size);
}
...
TEST_F(SecurityApplicationProxyFullSetupTest, shouldPassRemoveMembership)
{
    ...
    EXPECT_EQ(ER_OK, alljoyn_securityapplicationproxy_removemembership(m_managedAppSecurityApplicationProxy, (const uint8_t*)serial.c_str(), serial.size(), m_certIds.ids[0].issuerPublicKey, nullptr, 0U));
...
TEST_F(SecurityApplicationProxyFullSetupTest, shouldPassInstallManifests)
{
    ...
    ASSERT_EQ(ER_OK, alljoyn_securityapplicationproxy_signmanifest(s_validManagedAppManifestTemplate, m_managedAppIdentityCertificate, m_securityManagerPrivateKey, &m_signedManifestXml));
