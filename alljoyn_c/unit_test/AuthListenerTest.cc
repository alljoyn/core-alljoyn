...
class DefaultAuthListenerWithPasswordTest : public DefaultAuthListenerNoPasswordTest {
  public:
    ...
    virtual void SetUp()
    {
        ...
        ASSERT_EQ(ER_OK, alljoyn_authlistener_setpassword(m_defaultAuthListener, s_someValidPassword, SIZEOF_ARRAY(s_someValidPassword)));
    }
};
...
{
    EXPECT_EQ(ER_OK, alljoyn_authlistener_setpassword(m_defaultAuthListener, s_someValidPassword, SIZEOF_ARRAY(s_someValidPassword)));
}

TEST_F(DefaultAuthListenerNoPasswordTest, shouldFailSetAuthListenerShorterThan4BytesPassword)
{
    EXPECT_EQ(ER_BAD_ARG_2, alljoyn_authlistener_setpassword(m_defaultAuthListener, s_someInvalidPassword, SIZEOF_ARRAY(s_someInvalidPassword)));
}
