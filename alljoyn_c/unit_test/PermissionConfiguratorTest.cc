...
TEST_F(PermissionConfiguratorPreClaimTest, shouldPassSignCertificate)
{
    ...
    ASSERT_EQ(ER_OK, alljoyn_permissionconfigurator_signcertificate(m_configuratorUnderTest, (AJ_PCSTR)unSignedCert.c_str(), &signedCert));
    EXPECT_EQ(ER_OK, cert.LoadPEM(qcc::String(signedCert)));
    EXPECT_STRNE(signedCert, unSignedCert.c_str());
}
