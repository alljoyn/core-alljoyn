...
TEST(AboutDataTest, DISABLED_DefaultLanguageNotSpecified) {
    QStatus status = ER_FAIL;
    alljoyn_aboutdata aboutData = alljoyn_aboutdata_create_empty();
    status = alljoyn_aboutdata_setdevicename(aboutData, "Device Name", "en");
    EXPECT_EQ(ER_ABOUT_DEFAULT_LANGUAGE_NOT_SPECIFIED, status) << "  Actual Status: "
                                                               << QCC_StatusText(status);

    status = alljoyn_aboutdata_setappname(aboutData, "Application Name", "en");
    EXPECT_EQ(ER_ABOUT_DEFAULT_LANGUAGE_NOT_SPECIFIED, status) << "  Actual Status: "
                                                               << QCC_StatusText(status);

    status = alljoyn_aboutdata_setmanufacturer(aboutData, "Manufacturer Name", "en");
    EXPECT_EQ(ER_ABOUT_DEFAULT_LANGUAGE_NOT_SPECIFIED, status) << "  Actual Status: "
                                                               << QCC_StatusText(status);
