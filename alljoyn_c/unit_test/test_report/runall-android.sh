...
function Usage() {
	set +ex
	echo >&2 "
Runs GTest executables created by scons in alljoyn_c, on an Android device using adb.

Usage: $(basename -- "$0") [ -s -d alljoyn_dist ] [ -t alljoyn_test ] [-c configfile ] -- \"adb device\" [ ajctest ]
where
	...
	ajctest 	# simple file name(s) of the gtest exe(s) to be run (gtest exe files expected in -t path/c/bin)
