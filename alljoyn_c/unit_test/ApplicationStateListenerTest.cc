...
class ApplicationStateListenerTest : public testing::Test {
  ...
  public:
    ...
    {
        callbacksWithNullStateCallback.state = nullptr;
        nonNullCallbacks.state = someCallback;
        callbacksPassingPublicKeyToContext.state = passKeyToContextCallback;
        callbacksPassingApplicationStateToContext.state = passApplicationStateToContextCallback;
        callbacksPassingBusNameToContext.state = passBusNameToContextCallback;
    }
...
{
    // Workaround for "EXPECT_NO_THROW" not compiling under Linux.
