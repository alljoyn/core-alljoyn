...
size_t AJ_CALL alljoyn_interfacedescription_getdescriptionlanguages(const alljoyn_interfacedescription iface, const char** languages, size_t num)
{
    QCC_DbgTrace(("%s", __FUNCTION__));
    size_t count = 0;
    ajn::Translator* translator = ((ajn::InterfaceDescription*)iface)->GetDescriptionTranslator();
...
QCC_BOOL AJ_CALL alljoyn_interfacedescription_getmemberargannotation(const alljoyn_interfacedescription iface, const char* member, const char* argName, const char* name, char* value, size_t* value_size)
{
    ...
    QCC_BOOL return_value = QCC_FALSE;
    qcc::String out_val;
    bool b = ((ajn::InterfaceDescription*)iface)->GetArgAnnotation(member, argName, name, out_val);
