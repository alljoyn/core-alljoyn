...
QStatus AJ_CALL alljoyn_authlistener_setpassword(alljoyn_authlistener listener, const uint8_t* password, size_t passwordSize)
{
    QCC_DbgTrace(("%s", __FUNCTION__));

    return ((ajn::AuthListenerCallbackC*)listener)->SetPassword(password, passwordSize);
}
