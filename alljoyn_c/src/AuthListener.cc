...
QStatus AJ_CALL alljoyn_authlistener_setpassword(alljoyn_authlistener listener, AJ_PCSTR password)
{
    QCC_DbgTrace(("%s", __FUNCTION__));
    size_t passwordSize = (nullptr != password) ? strlen(password) : 0;

    // Workaround until ASACORE-3037 is resolved. C++ uses and unusual data type for passwords.
    return ((ajn::AuthListenerCallbackC*)listener)->SetPassword((const uint8_t*)password, passwordSize);
}
