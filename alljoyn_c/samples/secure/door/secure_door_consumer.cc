...
static Provider* NewProvider()
{
    ...
    provider->m_providerBusName = nullptr;
...
static void InsertNewProvider(AJ_PCSTR providerBusName)
{
    ...
    size_t providerBusNameLength = strlen(providerBusName);
    current->m_providerBusName = (AJ_PSTR)malloc(providerBusNameLength + 1);
    memcpy(current->m_providerBusName, providerBusName, providerBusNameLength + 1);
}
