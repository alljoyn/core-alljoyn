...
var alljoyn = (function () {
    ...
    that.start = function () {
        try {
            ...
            /* Create and connect all objects and listeners of the service */
            ...
            proxyBusObject = new BasicClientBusObject(busAtt, OBJECT_PATH);
    ...
    that.joinSession = function joinSession() {
    ...
    that.proxyRemoteConnect = function proxyRemoteConnect() {
        proxyBusObject.proxyBusObject.introspectRemoteObjectAsync(null).then(function (introResults) {
            if (AllJoyn.QStatus.er_OK == introResults.status) {
                OutputLine("Introspection of the service bus object was successful.");
                proxyBusObject.callCatMethod();
            } else {
