...
void MyAllJoynCode::enableEvent(EventInfo* event)
{
    QStatus status = ER_OK;
    ...
    if (eventMember) {
        status =  mBusAttachment->RegisterSignalHandler(this,
                                                        static_cast<MessageReceiver::SignalHandler>(&MyAllJoynCode::EventHandler),
                                                        eventMember,
                                                        NULL);
...
void MyAllJoynCode::EventHandler(const ajn::InterfaceDescription::Member* member, const char* srcPath, ajn::Message& msg)
{
    JNIEnv* env;
    jint jret = vm->GetEnv((void**)&env, JNI_VERSION_1_2);
    if (JNI_EDETACHED == jret) {
