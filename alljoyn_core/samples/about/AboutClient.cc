...
void printAboutDataForLanguage(AboutData& aboutData, const char* language, int tabLevel)
{
    ...
    for (size_t i = 0; i < numFields; ++i) {
        ...
        MsgArg* fieldArg;
        aboutData.GetField(fields[i], fieldArg, language);
        std::cout << "\t";
...
QStatus printAboutData(AboutProxy& aboutProxy) {
    MsgArg aboutArg;
    QStatus status = aboutProxy.GetAboutData("en", aboutArg);
    if (status != ER_OK) {
        std::cout << "printAboutData: AboutProxy.GetAboutData failed(" << status << ")" << std::endl;
        return status;
    }
    ...
    if (langCount > 1) {
        ...
        for (size_t l = 0; l < langCount; ++l) {
            if (strcmp(defaultLanguage, langs[l]) != 0) {
