...
class Observer {
  public:
     ...
     * @param optionalInterfaces     Set of additional interfaces that may be implemented
     *                               by a discovered object. If these are present, the
     *                               Observer-managed proxy bus object will support them.
     * @param numOptionalInterfaces  number of elemetns in the optionalInterfaces array.
     *
     * Some things to take into account:
     *   - the Observer will only discover objectst that are announced through About.
     *   - the interface names in mandatoryInterfaces and optionalInterfaces must correspond
     *     with InterfaceDescriptions that have been registered with the bus attachment
     *     before creation of the Observer.
     *   - mandatoryInterfaces must not be empty or NULL
     *   - optionalInterfaces may be empty or NULL
     */
    Observer(BusAttachment& bus,
             const char* mandatoryInterfaces[], size_t numMandatoryInterfaces,
             const char* optionalInterfaces[], size_t numOptionalInterfaces);
    ...
    class Listener {
      public:
         ...
         *
         * @param object a proxy bus object supporting all mandatory interfaces,
         *               and as many of the optional interfaces as the object
         *               implements.
         */
    ...
    /* All methods below that return ManagedProxyBusObjects will return an invalid
       object if appropriate (if there is no object with this ObjectId or when the
       iteration is finished).
       This can be checked with the ProxyBusObject::IsValid() method */
