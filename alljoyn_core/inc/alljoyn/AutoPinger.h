...
class AutoPinger : public qcc::AlarmListener {
  public:
     ...
     *                 ER_FAIL: group did not exist
     */
    QStatus RemoveDestination(const qcc::String& group, const qcc::String& destination);
  ...
  private:
    ...
    //std::map<qcc::String, std::unique_ptr<PingGroup> > pingGroups; /* Temporarily disable unique_ptr because not available on all platforms */
    std::map<qcc::String, PingGroup*> pingGroups;

    bool pausing;
    AutoPingAsyncCB* pingCallback;
};
}
#undef QCC_MODULE
#endif /* AUTOPINGER_H_ */
