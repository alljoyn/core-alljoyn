...
class _Message {
  ...
  public:
    ...
    uint64_t GetCryptoValue() const {
        assert(hdrFields.field[ALLJOYN_HDR_FIELD_CRYPTO_VALUE].typeId == ALLJOYN_UINT64);
     ...
     * This determines if the encrypted flag has been set and the Auth Version is 3 or greater.
     *
     * @return
     *      - true if the 64 bit crypto Number is required.
     *      - false if otherwise.
     */
    bool RequiresCryptoValue() const { return (encrypt & (MIN_AUTH_VERSION_USE_CRYPTO_VALUE <= (GetAuthVersion() >> 16))); }
     ...
     * Note that this will not be set before GUIDs are exchanged.
     *
