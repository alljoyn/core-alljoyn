...
static const uint8_t ALLJOYN_FLAG_ALLSESSION_BROADCAST = (ALLJOYN_FLAG_GLOBAL_BROADCAST | 0x02);
 ...
 * so we've had to improvise. The AUTO_START bit is meaningless in the context
 * of signals, so the reuse of this bit ought to be safe. The combination with
 * GLOBAL_BROADCAST ensures that older routers do not simply drop the message,
 * but rather treat it as a GLOBAL_BROADCAST message. That ensures that, in the
 * presence of legacy routers, the signal gets delivered _at least_ to all
 * intended recipients, but potentially also to some unintended receivers.
 */
// @}
