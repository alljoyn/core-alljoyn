...
class ProxyBusObject : public MessageReceiver {
  ...
  private:
     ...
     *       - An error status otherwise.
     */
    QStatus ParseLegacyXml(const char* xml, const char* identifier = NULL);
     ...
     *       - An error status otherwise.
     */
    QStatus GetXmlsWithDescriptions(XmlToLanguageMap* xmls);
     ...
     * @param[out] xmls The map to store the XMLs.
     *
     * @return
     *       - ER_OK if the XML was successfully transformed to qcc::XmlParseContext and stored.
     *       - An error status otherwise.
     */
    QStatus AddDescriptionXmlToMap(const qcc::String& xml, const qcc::String& languageTag, XmlToLanguageMap* xmls) const;
     ...
     *
     * @param xml[in] The introspection XML without descriptions.
     * @param xmlsWithDescriptions[in] Map containing XMLs with descriptions.
     * @param identifier[in] An optional identifying string to include in error logging messages.
     *
     * @return
     *       - ER_OK if the XMLs were successfully parsed.
     *       - An error status otherwise.
     */
    QStatus ParseXmlAndDescriptions(const char* xml, const XmlToLanguageMap* xmlsWithDescriptions, const char* identifier = nullptr);
};
