  ...
  public:
    ...
    class Marshaller {
      public:
        ...
        *      - Error code if fail.
        */
        virtual QStatus MarshalManifestTemplate(const PermissionPolicy::Rule* rules, size_t rulesCount, std::vector<uint8_t>& buf)
        {
            QCC_UNUSED(rules);
            QCC_UNUSED(rulesCount);
            QCC_UNUSED(buf);
            return ER_NOT_IMPLEMENTED;
        }
         ...
         *      - Error code if fail.
         */
        virtual QStatus UnmarshalManifestTemplate(const uint8_t* buf, size_t size, std::vector<PermissionPolicy::Rule>& rules)
        {
            QCC_UNUSED(buf);
            QCC_UNUSED(size);
            QCC_UNUSED(rules);
     ...
     *      - error code if fail
     */
    static QStatus RulesToMsgArg(const Rule* rules, size_t count, MsgArg& msgArg);

    /**
     * Parse the MsgArg object to retrieve the policy and manifest rules.
     ...
     *      - error code if fail
     */
    static QStatus MsgArgToRules(const MsgArg& msgArg, Rule** rules, size_t* count);

    /**
    * Parse the MsgArg object to retrieve the manifest template rules.
    * @param        msgArg  The MsgArg containing the manifest template rules.
...
class _Manifest {
  ...
  public:
    ...
    /** MsgArg signature for manifest and policy rules array. */
    static AJ_PCSTR s_RulesArrayMsgArgSignature;
