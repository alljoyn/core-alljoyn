...
QStatus UDPTransport::GetListenAddresses(const SessionOpts& opts, std::vector<qcc::String>& busAddrs) const
{
    ...
    if (m_wildcardIfaceProcessed || m_wildcardAddressProcessed) {
        ...
        for (std::map<qcc::String, InterfaceInfo>::const_iterator it = m_requestedInterfaces.begin();
            ...
            if ((it->first != "*") && !RequestedInterfaceContainsAddress(it->first, IPAddress(ADDR4_DEFAULT)) &&
                !RequestedInterfaceContainsAddress(it->first, IPAddress(ADDR6_DEFAULT))) {
...
void UDPTransport::ManageEndpoints(uint32_t authTimeout, uint32_t sessionSetupTimeout)
{
    ...
    while (i != m_preList.end()) {
        ...
        QCC_DbgPrintf(("UDPTransport::ManageEndpoints(): Moving endpoint with connId == %d. from m_preList to m_authList", ep->GetConnId()));
#ifndef NDEBUG
    ...
    if (IsRunning() == false || m_stopping == true) {
...
#endif
            /*
             * The Endpoint Stop method will set m_manage and alert
             * the maintenance thread whenever it is called in case it
             * is called from a third party (Daemon Router, for
             * example).  Be careful to only call stop if the endpoint
             * is in a "lower" state (hasn't progressed to at least
             * EP_STOPPING) or we end up efectively polling for
             * completion as we Stop() the endpoint which alerts the
             * maintenance thread which runs and causes us to Stop()
             * the endpoint, which ...
             */
        ...
        while (i != m_authList.end()) {
            ...
            ep->SetEpWaitEnable(false);
            ep->Stop();
            QCC_DbgPrintf(("UDPTransport::ManageEndpoints(): Moving endpoint with connId == %d. from m_authList to m_endpointList", ep->GetConnId()));
    ...
    while (i != m_endpointList.end()) {
        ...
        if (ep->IsEpStopping() || ep->IsEpWaiting() || ep->IsEpJoined()) {
            ...
            if (ep->IsEpWaiting()) {
                ...
                if (tRemaining < 0) {
                    QCC_LogError(ER_TIMEOUT, ("UDPTransport::ManageEndpoints(): Max time in EP_WAITING Elapsed"));
                    ep->SetEpWaitEnable(false);
                    ep->SetEpStopping();
                    stream->Disconnect(false, ER_UDP_LOCAL_DISCONNECT);
...
bool UDPTransport::AcceptCb(ArdpHandle* ardpHandle, qcc::IPAddress ipAddr, uint16_t ipPort, ArdpConnRecord* conn, uint8_t* buf, uint16_t len, QStatus status)
{
    ...
    status = activeHello->HelloReply(true, udpEp->GetUniqueName(),
                                     (nameTransfer == SessionOpts::SLS_NAMES) ? SessionOpts::SLS_NAMES : SessionOpts::ALL_NAMES);
    if (status != ER_OK) {
...
void UDPTransport::DoConnectCb(ArdpHandle* ardpHandle, ArdpConnRecord* conn, uint32_t connId, bool passive, uint8_t* buf, uint16_t len, QStatus status)
{
    ...
    while (i != m_preList.end()) {
        UDPEndpoint ep = *i;
        QCC_DbgPrintf(("UDPTransport::DoConnectCb(): Moving endpoint with connId == %d. from m_preList to m_authList", ep->GetConnId()));
        m_authList.insert(ep);
        m_preList.erase(i);
        i = m_preList.begin();
    }
    ...
    if (passive) {
...
#endif
                QCC_DbgPrintf(("UDPTransport::DoConnectCb(): Adding endpoint with connId == %d. to m_endpointList", ep->GetConnId()));
                m_endpointList.insert(ep);
                m_connLock.Unlock(MUTEX_CONTEXT);
        ...
        QCC_DbgPrintf(("UDPTransport::DoConnectCb(): adding endpoint with connId == %d. to m_endpointList", udpEp->GetConnId()));
...
void UDPTransport::ExitEndpoint(uint32_t connId)
{
    ...
    QCC_DbgPrintf(("UDPTransport::ExitEndpoint(): sending EXIT request to exit dispatcher for connId == %d", connId));
    m_exitWorkerCommandQueueLock.Lock(MUTEX_CONTEXT);
    m_exitWorkerCommandQueue.push(entry);
    m_exitWorkerCommandQueueLock.Unlock(MUTEX_CONTEXT);
    m_exitDispatcher->Alert();
...
void UDPTransport::RecvCb(ArdpHandle* ardpHandle, ArdpConnRecord* conn, ArdpRcvBuf* rcv, QStatus status)
{
    ...
    if (m_dispatcher == NULL) {
         ...
         * here.
         */
        QCC_DbgPrintf(("UDPTransport::DispatcherThread::RecvCb(): Orphaned Recv_Cb for conn == %p ignored", conn));
...
void UDPTransport::SendCb(ArdpHandle* ardpHandle, ArdpConnRecord* conn, uint8_t* buf, uint32_t len, QStatus status)
{
    ...
    QCC_DbgPrintf(("UDPTransport::SendCb(): sending SEND_CB request for connId == %d. to dispatcher)", entry.m_connId));
    m_workerCommandQueueLock.Lock(MUTEX_CONTEXT);
    m_workerCommandQueue.push(entry);
    m_workerCommandQueueLock.Unlock(MUTEX_CONTEXT);
    m_dispatcher->Alert();
...
void* UDPTransport::Run(void* arg)
{
    ...
    for (;;) {
        ...
        QCC_DbgPrintf(("UDPTransport::Run(): Defer ManageEndpoints() tMin=%d. ms", tMin));

        int64_t tRemaining = tLastManage + UDP_ENDPOINT_MANAGEMENT_TIMER - tNow;
        QCC_DbgPrintf(("UDPTransport::Run(): Defer ManageEndpoints() tRemaining=%d. ms", tRemaining));

        if (m_manage != STATE_MANAGED || tRemaining < 0) {
            QCC_DbgPrintf(("UDPTransport::Run(): ManageEndpoints() suggested"));

            /*
             * Get defensive about various threads bugging us constantly.  There
             * is no reason to run ManageEndpoints faster than endpoints can be
             * created and destroyed -- so if we were just here less than some
             * minimum time ago, don't run the code but make sure it is scheduled
             * to run after that time.
             */
            if (tMin < 0) {
                QCC_DbgPrintf(("UDPTransport::Run(): ManageEndpoints() due"));

                /*
                 * Set m_manage to STATE_MANAGED before calling ManageEndpoints in
                 * case something happens while ManageEndpoints is half-way done
                 * which requires addiional endpoint management.
                 */
                m_manage = STATE_MANAGED;
                QCC_DbgPrintf(("UDPTransport::Run(): ManageEndpoints()"));
                ...
                uint32_t tManage = UDP_ENDPOINT_MANAGEMENT_MIN;
                QCC_DbgPrintf(("UDPTransport::Run(): Defer ManageEndpoints() for %d. ms", tManage));
                maintenanceTimerEvent.ResetTime(tManage, 0);
            }
        }
...
QStatus UDPTransport::NormalizeListenSpec(const char* inSpec, qcc::String& outSpec, map<qcc::String, qcc::String>& argMap) const
{
     ...
     *
     * That's all.  We still allow "addr=0.0.0.0,port=9955,family=ipv4"
     */
...
QStatus UDPTransport::Connect(const char* connectSpec, const SessionOpts& opts, BusEndpoint& newEp)
{
     ...
     * bound to INADDR_ANY (which means that we got a listen on 0.0.0.0 or ::);
     ...
     * These are the older values of nameTransfer to be sent in BusHello.
     */
    status = hello->HelloMessage(true, m_bus.GetInternal().AllowRemoteMessages(),
                                 (opts.nameTransfer == SessionOpts::SLS_NAMES) ? SessionOpts::SLS_NAMES : SessionOpts::ALL_NAMES);
    if (status != ER_OK) {
        status = ER_UDP_BUSHELLO;
    ...
    /*
     * We were a thread waiting on an action but no longer.  It is possible that
     * the endpoint manager is waiting for this thread to leave, so tell it to
     * runs.
     */
    m_manage = STATE_MANAGE;
    Alert();
...
QStatus UDPTransport::DoStartListen(qcc::String& normSpec)
{
     ...
     * If given "*" or "0.0.0.0" or "::", this means that the TCP Transport will
     ...
     *     iface                  Action
     *     ----------             -----------------------------------------
     * 1.  *                      Listen on 0.0.0.0 or :: and advertise/discover
     ...
     *     'a:b:c:d'              or when it appears on the network and
    ...
    if (!iface.empty()) {
        ...
        m_listenPortMap[iface] = listenPort;
    } else if (addr.Size()) {
...
void UDPTransport::QueueUpdateRouterAdvertisementAndDynamicScore()
{
    ...
    enableAdvListenRequest.m_requestParam = m_routerName;
    enableAdvListenRequest.m_requestParamOpt = true;
    enableAdvListenRequest.m_requestTransportMask = TRANSPORT_UDP;

    ListenRequest disableAdvListenRequest;
    disableAdvListenRequest.m_requestOp = DISABLE_ADVERTISEMENT_INSTANCE;
    disableAdvListenRequest.m_requestParam = m_routerName;
    disableAdvListenRequest.m_requestTransportMask = TRANSPORT_UDP;
...
QStatus UDPTransport::SetupSocket(qcc::SocketFd& socketFd, const qcc::IPAddress& addr, uint16_t port, uint32_t scopeId)
{
    ...
    if (status != ER_OK) {
        QCC_LogError(status, ("UDPTransport::SetupSocket(): SetReusePort() failed"));
...
#ifndef NDEBUG
    sndSize = 0;
    QCC_VERIFY(qcc::GetSndBuf(socketFd, sndSize) == ER_OK);
...
#ifndef NDEBUG
    rcvSize = 0;
    QCC_VERIFY(qcc::GetRcvBuf(socketFd, rcvSize) == ER_OK);
...
bool UDPTransport::RequestedInterfaceContainsAddress(const String& iface, const IPAddress& addr) const
{
...
void UDPTransport::AddInterfaceAddressToReplacedList(const IPAddress& addr, const InterfaceInfo& ifaceInfo, list<String>& replacedList) const
{
    String replacedAcceptingSpec = "udp:addr=" + addr.ToString() + ",port=" + U32ToString(ifaceInfo.m_acceptingPort);
    replacedList.push_back(replacedAcceptingSpec);
    String replacedActiveSpec = "udp:addr=" + addr.ToString() + ",port=" + U32ToString(ifaceInfo.m_activePort);
    replacedList.push_back(replacedActiveSpec);
}

void UDPTransport::RequestedInterfacesCleanup(const multimap<String, IPAddress>& ifMap, list<qcc::String>& replacedList)
{
    ...
    //if m_requestedInterfaces contains interface entry which is not present in liveInterfaces then remove all associated IPAddresses
    for (map<String, InterfaceInfo>::iterator ifaceEntry = m_requestedInterfaces.begin(); ifaceEntry != m_requestedInterfaces.end(); ++ifaceEntry) {
        if (find(liveInterfaces.begin(), liveInterfaces.end(), ifaceEntry->first) == liveInterfaces.end()) {
            QCC_DbgTrace(("UDPTransport::RequestedInterfacesCleanup(), removing from m_requestedInterfaces[%s] all addresses", ifaceEntry->first.c_str()));
            for (vector<IPAddress>::iterator addrEntry = ifaceEntry->second.m_addresses.begin(); addrEntry != ifaceEntry->second.m_addresses.end(); ++addrEntry) {
                AddInterfaceAddressToReplacedList(*addrEntry, ifaceEntry->second, replacedList);
                QCC_DbgTrace(("UDPTransport::RequestedInterfacesCleanup(), removing m_requestedInterfaces[%s] address entry: %s",
    ...
    for (vector<String>::iterator it = liveInterfaces.begin(); it != liveInterfaces.end(); ++it) {
        if (m_requestedInterfaces.find(*it) == m_requestedInterfaces.end()) {
            QCC_DbgTrace(("UDPTransport::RequestedInterfacesCleanup(), m_requestedInterfaces[%s] does not exists, advancing the loop", (*it).c_str()));
            continue;
        }
        ...
        for (vector<IPAddress>::iterator addrEntry = m_requestedInterfaces[*it].m_addresses.begin();
            ...
            if (find_if(ifMapInterfaceAddresses.first, ifMapInterfaceAddresses.second,
                ...
                AddInterfaceAddressToReplacedList(*addrEntry, m_requestedInterfaces[*it], replacedList);
                QCC_DbgTrace(("UDPTransport::RequestedInterfacesCleanup(), removing m_requestedInterfaces[%s].m_addresses entry: %s",
...
void UDPTransport::HandleNetworkEventInstance(ListenRequest& listenRequest)
{
    ...
    bool wildcardAddressRequested = ((m_requestedAddresses.find(ADDR4_DEFAULT) != m_requestedAddresses.end()) ||
                                     (m_requestedAddresses.find(ADDR6_DEFAULT) != m_requestedAddresses.end()));
    ...
    for (std::multimap<qcc::String, qcc::IPAddress>::const_iterator it = ifMap.begin(); it != ifMap.end(); it++) {
        ...
        if (!wildcardIfaceRequested && currentIfaceRequested) {
            if (RequestedInterfaceContainsAddress(interface, address)) {
                ...
                if (fd != INVALID_SOCKET_FD) {
                    QCC_DbgPrintf(("UDPTransport::HandleNetworkEventInstance(): found already address %s on matching interface %s which has a valid listedFd %d => moving to the next entry in the loop",
                                   address.ToString().c_str(), interface.c_str(), fd
                                   ));
                    continue;
                } else {
                    QCC_DbgPrintf(("UDPTransport::HandleNetworkEventInstance(): found already address %s on matching interface %s but without valid listedFd => proceed",
        ...
        if (!wildcardAddressRequested && currentAddressRequested) {
            if (m_requestedAddresses[addressStr].m_interface == interface) {
                ...
                //this address has moved between interfaces, socket (if opened) needs to recreated so add this to cleanup list
        ...
        if (wildcardIfaceRequested) {
            //for wildcardIfaceRequested we want to setup both, ADDR4_DEFAULT and ADDR6_DEFAULT
            ...
            //for wildcardAddressRequested we want to setup only what was requested, IPv4 (0.0.0.0) or IPv6 (::)
            wildcardAddress = ((m_requestedAddresses.find(ADDR6_DEFAULT) != m_requestedAddresses.end()) ? ADDR6_DEFAULT : ADDR4_DEFAULT);
            listenAddr = qcc::IPAddress(wildcardAddress);
        ...
        /*
         * IPv4 addresses and unspecified IPv6 address ("::") has no scope,
        ...
        if (listenAddr.IsIPv6() && (listenAddr.ToString() != ADDR6_DEFAULT)) {
            ...
            int code = getaddrinfo(tmpaddr.c_str(), NULL, NULL, &resp);
            if (code == 0) {
                listenAddrScopeId = ((struct sockaddr_in6*)resp->ai_addr)->sin6_scope_id;
            } else {
                QCC_LogError(ER_FAIL, ("UDPTransport::HandleNetworkEventInstance(): getaddrinfo() failed for node '%s', reason: %s", tmpaddr.c_str(), gai_strerror(code)));
            }
        }
    ...
    /*
     * Check if we have some trash (stale IPs) on m_requestedInterfaces map and make a propoer cleanup.
    ...
    if (!wildcardAddressRequested && !wildcardIfaceRequested) {
        QCC_DbgPrintf(("UDPTransport::HandleNetworkEventInstance(): calling RequestedInterfacesCleanup()"));
        RequestedInterfacesCleanup(ifMap, replacedList);
    }
     ...
     * NOTE:
     * This needs to happen after cleanup (after replacedList is processed) so it might be that address has been
     * moved from one interface to another so it on both replacedList and addedList.
     * Also resources-wise this is a better approach.
     */
    if (!addedList.empty()) {
        m_listenFdsLock.Lock(MUTEX_CONTEXT);
        for (list<ListenFdEntry>::iterator it = addedList.begin(); it != addedList.end(); it++) {
            QCC_DbgPrintf(("UDPTransport::HandleNetworkEventInstance(): Adding normalized listen spec \"%s\", sockFd %d, accepting: %s to m_listenFds",
                           it->m_normSpec.c_str(), it->m_sockFd, (it->m_accepting ? "y" : "n")));
