...
void* UDPTransport::Run(void* arg)
{
    ...
    for (;;) {
        ...
        for (vector<Event*>::iterator i = signaledEvents.begin(); i != signaledEvents.end(); ++i) {
            ...
            if (readReady) {
                m_listenFdsLock.Lock(MUTEX_CONTEXT);
                QStatus findSocketStatus = IsSocketAccepting((*i)->GetFD(), socketIsAccepting);
                m_listenFdsLock.Unlock(MUTEX_CONTEXT);
                /* If we have incoming traffic from a socket which is not on the FD list,
                 * something very odd has happened.
                 */
                QCC_ASSERT(findSocketStatus == ER_OK && "UDPTransport::Run(): Incoming traffic from an unknown socket");
            }
            m_ardpLock.Lock(MUTEX_CONTEXT);
            if (socketReady) {
