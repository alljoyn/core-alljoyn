...
void IpNameServiceImpl::HandleProtocolQuery(MDNSPacket mdnsPacket, const qcc::IPEndpoint& remote, const qcc::IPEndpoint& local)
{
    ...
    /* Here we are checking if IPNS configuration (that is physical host interfaces filtered by
     * requested interfaces per TCP and UDP transport) are matching a response destination address.
     */
    ...
    if (!senderAddressMatchesOurLiveInterfaces) {
        QCC_DbgPrintf(("IpNameServiceImpl::HandleProtocolQuery: rejecting %s:%d with transport mask 0x%x", remote.GetAddress().ToString().c_str(), remote.GetPort(), completeTransportMask));
        return;
    }
    m_mutex.Lock(MUTEX_CONTEXT);
