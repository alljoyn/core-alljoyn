...
void IpNameServiceImpl::HandleProtocolQuery(MDNSPacket mdnsPacket, const qcc::IPEndpoint& remote, const qcc::IPEndpoint& local)
{
    ...
    TransportMask matchingTransportMask = TRANSPORT_NONE;
    for (auto& liveInterface : m_liveInterfaces) {
        ...
        if ((liveInterface.m_transportMask & completeTransportMask) != TRANSPORT_NONE) {
            ...
            matchingTransportMask |= liveInterface.m_transportMask;
            break;
        }
    }
    ...
    bool handled = false;
    for (list<IpNameServiceListener*>::iterator it = m_listeners.begin(); !handled && it != m_listeners.end(); ++it) {
        handled = (*it)->QueryHandler(matchingTransportMask, mdnsPacket, local, dst);
    }
    m_mutex.Lock(MUTEX_CONTEXT);
    m_protectListeners = false;
    if (handled) {
    ...
    HandleSearchQuery(matchingTransportMask, mdnsPacket, local, guid, dst);
