...
void IpNameServiceImpl::SendOutboundMessageQuietly(Packet packet)
{
    ...
    for (uint32_t i = 0; (m_state == IMPL_RUNNING) && (i < m_liveInterfaces.size()); ++i) {
        ...
        if (SameNetwork(interfaceAddressPrefixLen, m_liveInterfaces[i].m_address, destination.addr)) {
            ...
            //
            // FIXME: This is disabled for now untill following scenario is resolved:
            ...
            } else if (m_unreliableIPv4PortMap[TRANSPORT_INDEX_UDP].find("::") != m_unreliableIPv4PortMap[TRANSPORT_INDEX_UDP].end()) {
                unreliableTransportPort = m_unreliableIPv4PortMap[TRANSPORT_INDEX_UDP]["::"];
...
void IpNameServiceImpl::HandleProtocolMessage(uint8_t const* buffer, uint32_t nbytes, const qcc::IPEndpoint& remote, const qcc::IPEndpoint& local, int32_t interfaceIndex)
{
    ...
    if (local.port == 9956) {
        ...
        //
        if (!m_enableV1) {
            QCC_DbgPrintf(("IpNameServiceImpl::HandleProtocolMessage(): m_enableV1 not enabled, returning"));
            return;
        }
        //
        ...
        if (doAnyNetworkCallback) {
            ...
            for (uint32_t i = 0; (m_impl.m_state == IMPL_RUNNING) && (i < m_impl.m_liveInterfaces.size()); ++i) {
                if (m_impl.m_liveInterfaces[i].m_address.IsIPv4() || m_impl.m_liveInterfaces[i].m_address.IsIPv6()) {
                    ifMap.insert(pair<qcc::String, qcc::IPAddress> (m_impl.m_liveInterfaces[i].m_interfaceName, m_impl.m_liveInterfaces[i].m_address));
#ifndef NDEBUG
                    QCC_DbgPrintf(("IpNameServiceImpl::PacketScheduler: (iteration '%d') setting for interface '%s' address '%s'", i,
                            ...
                            for (uint32_t j = 0; j < m_impl.m_requestedInterfaces[transportIndex].size(); j++) {
                                for (std::multimap<qcc::String, qcc::IPAddress>::iterator it = ifMap.begin(); it != ifMap.end(); it++) {
                                    ...
                                    if ((m_impl.m_requestedInterfaces[transportIndex][j].m_interfaceName == name) ||
                                        ...
                                        transportIfMap.insert(pair<qcc::String, qcc::IPAddress> (name, addr));
                                    }
                                }
                            }
                        } else {
