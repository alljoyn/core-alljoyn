...
void IpNameServiceImpl::SendOutboundMessageQuietly(Packet packet)
{
    ...
    // we are filtering content of MDNS packets but also whether or not to skip
    // at all MDNS packet, based on the information which transport has been opened
    // by which interface interface, so we simply assume that the right thing to do
    // was done and we just send the message on out the interface corresponding
                                          ...
                                          const qcc::String& guid, const qcc::IPEndpoint& dst)
{
    ...
    QCC_DbgPrintf(("IpNameServiceImpl::HandleSearchQuery, src = %s, dst = %s, 0x%x", src.GetAddress().ToString().c_str(), dst.GetAddress().ToString().c_str(), completeTransportMask));
    MDNSResourceRecord* searchRecord = nullptr;
    if ((!mdnsPacket->GetAdditionalRecord("search.*", MDNSResourceRecord::TXT, MDNSTextRData::TXTVERS, &searchRecord)) || (nullptr == searchRecord)) {
        return false;
    }
