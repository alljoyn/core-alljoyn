...
void IpNameServiceImpl::SendOutboundMessageQuietly(Packet packet)
{
    ...
    for (uint32_t i = 0; (m_state == IMPL_RUNNING) && (i < m_liveInterfaces.size()); ++i) {
        ...
        if (SameNetwork(interfaceAddressPrefixLen, m_liveInterfaces[i].m_address, destination.addr)) {
            ...
            // If our router is configured on interface which has IPv4 and IPv6 addresses,
            // we will respond over IPv4 and IPv6 addresses so router B will get from us
            ...
            // on router B could make a decision on which address to connect based on it's
            // configuration (that is which sockets it has opened).
            /*
            ...
            /*
             * NOTE:
             * As for now there is always "haveIPv4address XOR haveIPv6address" due to disabled code above.
             * In case we have both, ipv4address and ipv6address, we should check if there could be a different
             * unreliableTransportPort for ipv4address and ipv6address and act accordingly.
             */
            RewriteVersionSpecific(msgVersion, packet, haveIPv4address, ipv4address, haveIPv4address, ipv6address,
