...
static void RetransmitTimerHandler(ArdpHandle* handle, ArdpConnRecord* conn, void* context)
{
    ...
    if (timer->retry > 1) {
        ...
        snd->fastRT = handle->config.fastRetransmitAckCounter + 1;
        ...
        QCC_DbgHLPrintf(("RetransmitTimerHandler seq=%u retries hit the limit: %d", ntohl(((ArdpHeader*)snd->hdr)->seq), handle->config.totalDataRetryTimeout / handle->config.initialDataTimeout));
        timer->retry = 0;
