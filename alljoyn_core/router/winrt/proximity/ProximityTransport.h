...
#ifndef _ALLJOYN_PROXIMITYTRANSPORT_H
#define _ALLJOYN_PROXIMITYTRANSPORT_H

#ifndef __cplusplus
#error Only include ProximityTransport.h in C++ code.
#endif
 ...
 *
 * The ProximityTransport class has different incarnations depending on whether or not
...
class ProximityTransport : public Transport, public _RemoteEndpoint::EndpointListener, public qcc::Thread, public ProximityListener {
  ...
  public:
     ...
     */
    ProximityTransport(BusAttachment& bus);

    /**
     * Destructor
     */
    virtual ~ProximityTransport();
     ...
     *
     * @param listenSpec  Transport specific key/value arguments that specify the physical interface to listen on.
     *                    - Valid transport is @c "proximity". All others ignored.
     *                    - Valid keys are:
     *                        - @c addr = IP address of server to connect to.
     *                        - @c port = Port number of server to connect to.
     *
    ...
    /**
     * Callback for ProximityEndpoint exit.
     *
  ...
  private:
     ...
     *
     * The server accept loop (executing in ProximityTransport::Run() uses the
    ...
    FoundCallback m_foundCallback;  /**< Called by ProximityNameService when new busses are discovered */
...
#endif // _ALLJOYN_ProximityTransport_H
