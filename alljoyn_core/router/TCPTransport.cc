    ...
    while (i != m_endpointList.end()) {
        ...
        if (authState == _TCPEndpoint::AUTH_DONE) {
            ...
            if ((ep->GetFeatures().isBusToBus && !ep->IsSessionRouteSetUp()) && (ep->GetStartTime() + sessionSetupTimeout < tNow)) {
                ...
                QCC_DbgHLPrintf(("TCPTransport:: Stopping endpoint that timedout waiting for routing to be set up %s.", ep->GetUniqueName().c_str()));
                ep->Stop();
            }
        }
...
void TCPTransport::EnableAdvertisementInstance(ListenRequest& listenRequest)
{
    ...
    QStatus status = IpNameService::Instance().AdvertiseName(TRANSPORT_TCP, listenRequest.m_requestParam, listenRequest.m_requestParamOpt, TRANSPORT_TCP);
    if (status != ER_OK) {
...
{
    QCC_DbgTrace(("TCPTransport::%s(%s)", __FUNCTION__, iface.c_str()));
    for (const auto& addressEntry : m_requestedAddresses) {
        if (addressEntry.second == iface) {
            auto itPortMap = m_requestedAddressPortMap.find(addressEntry.first);
            if (itPortMap != m_requestedAddressPortMap.end()) {
                String listenSpec = "tcp:addr=" + addressEntry.first + ",port=" + U32ToString(itPortMap->second);
    ...
    for (const auto& interfaceEntry : m_requestedInterfaces) {
        if (interfaceEntry.first == iface) {
            String listenSpec = "tcp:addr=" + interfaceEntry.second.GetAddress().ToString() + ",port=" + U32ToString(interfaceEntry.second.GetPort());
            QCC_DbgPrintf(("TCPTransport::%s calling DoStopListen(%s)", __FUNCTION__, listenSpec.c_str()));
            DoStopListen(listenSpec);
        }
    }
...
qcc::SocketFd TCPTransport::GetMatchingListenFdEntry(const qcc::String& listenSpec)
{
    ...
    for (const auto& listenFdsEntry : m_listenFds) {
        if (listenFdsEntry.first == listenSpec) {
            socket = listenFdsEntry.second;
            break;
        }
    }
    m_listenFdsLock.Unlock(MUTEX_CONTEXT);
