...
void SessionlessObj::ListImplicitRules()
{
#ifndef NDEBUG
    QCC_DbgPrintf(("IMPLICIT RULE TABLE"));
    QCC_DbgPrintf(("==================="));
    for (ImplicitRuleIterator irit = implicitRules.begin(); irit != implicitRules.end(); ++irit) {
        QCC_DbgPrintf(("implicit rule='%s'", irit->ToString().c_str()));
        for (std::vector<RuleIterator>::iterator erit = irit->explicitRules.begin();
             erit != irit->explicitRules.end();
             ++erit) {
            QCC_DbgPrintf(("\tendpoint='%s' rule='%s'", (*erit)->first.c_str(), (*erit)->second.ToString().c_str()));
        }
    }
    QCC_DbgPrintf(("=====[ DONE ]======"));
#endif
