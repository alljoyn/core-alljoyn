                                            ...
                                            const SessionId detachId)
{
    ...
    if (isBroadcast) {
        ...
        if (!add) {
            ...
            add = add || (srcInSession && destInSession && !destIsVirtual && (((src != dest) || selfJoin)) && dest->AllowRemoteMessages());
        }
    }
...
QStatus DaemonRouter::PushMessage(Message& msg, BusEndpoint& src)
{
    ...
    QCC_ASSERT(src->GetEndpointType() != ENDPOINT_TYPE_VIRTUAL);
    /*
    ...
    for (vector<BusEndpoint>::const_iterator it = allEps.begin(); it != allEps.end(); ++it) {
        ...
        if (!add) {
            ...
            policyRejected = true;
            skippedEndpoints.insert(dest->GetUniqueName());
            continue;
        }
#endif
    ...
    QCC_ASSERT(status != ER_NONE);

    /*
     * Updating local endpoints with possible error message
     */
