...
void* Test::Run(void* arg)
{
    ...
    status = qcc::Bind(sock, ipToConnectTo, atoi(g_localport));
    if (status != ER_OK) {
        ...
        for (std::vector<qcc::Event*>::iterator i = signaledEvents.begin(); i != signaledEvents.end(); ++i) {
            if (*i == &stopEvent) {
                ...
                if (connectSent == false) {
                    ArdpConnRecord* conn;
                    if (ER_OK == ARDP_Connect(ardpHandle, sock, qcc::IPAddress(ipToConnectTo.IsIPv4() ? "0.0.0.0" : "::"),
                ...
                uint32_t ms;
                ARDP_Run(ardpHandle, sock, true, false, true, &ms);
            }
        }
    }
