...
class UDPTransport : public Transport, public _RemoteEndpoint::EndpointListener, public qcc::Thread {
  ...
  private:
     ...
     */
    bool RequestedInterfaceContainsAddress(const qcc::String& iface, const qcc::IPAddress& addr) const;
    qcc::SocketFd GetMatchingListenFdEntry(const qcc::String& listenSpec) const;
    void AddInterfaceAddressToReplacedList(const qcc::IPAddress& addr, const InterfaceInfo& ifaceInfo, std::list<qcc::String>& replacedList) const;
    void RequestedInterfacesCleanup(const std::multimap<qcc::String, qcc::IPAddress>& ifMap, std::list<qcc::String>& replacedList);
