...
int CDECL_CALL main()
{
...
#endif
    {
        ...
        if (ER_OK == status) {
            exampleIntf->AddMethod("Echo", "s", "s", "input_arg,return_arg");
            exampleIntf->AddSignal("Chirp", "s", "sound", MEMBER_ANNOTATE_SESSIONCAST);
            exampleIntf->AddProperty("Volume", "i", PROP_ACCESS_RW);
