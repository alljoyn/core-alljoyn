...
class MultiPeerTestObject : public BusObject {
  public:
    MultiPeerTestObject(BusAttachment& bus, qcc::String path, String _interfaces) :
        ...
        assert(ER_OK == AddMethodHandler(intf->GetMember(METHOD),
                                         static_cast<MessageReceiver::MethodHandler>(&MultiPeerTestObject::HandleIdentify)));

        assert(ER_OK == AddMethodHandler(intf->GetMember(METHOD_OBSUPDATER),
    ...
    void HandleIdentify(const InterfaceDescription::Member* member, Message& message) {
        ...
        QStatus status;
        if (ER_OK != (status = MethodReply(message, args, 2))) {
            QCC_LogError(status, ("Failed to handle identity"));
        }
    }
    ...
    void UpdateObservedSoFarBy(const InterfaceDescription::Member* member, Message& message) {
        ...
        if (strcmp(option, "insert") == 0) {
            assert(observedSoFarBy.find(pid) == observedSoFarBy.end());     // Double observation
...
class Participant : public SessionPortListener, public SessionListener {
  public:
        ...
        port(42), aboutData("en"), aboutObj(bus), intfName(_intfName)
    {
        ...
        assert(ER_OK == bus.CreateInterface(intfName.c_str(), intf));
        assert(intf != NULL);
        assert(ER_OK == intf->AddMethod(METHOD, "", "ss", "busname,path"));
        assert(ER_OK == intf->AddMethod(METHOD_OBSUPDATER, "su", "", "op,pid"));
        intf->Activate();
    ...
    void StartBus() {
        ...
        assert(ER_OK == bus.Connect(busAddress.c_str()));
        assert(ER_OK == bus.BindSessionPort(port, opts, *this));
        uniqueBusName = bus.GetUniqueName();
    }

    void Announce() {
        assert(ER_OK == aboutObj.Announce(port, aboutData));
    }
    ...
    void StopBus() {
        assert(ER_OK == bus.Disconnect());
        assert(ER_OK == bus.Stop());
        assert(ER_OK == bus.Join());
    }
    ...
    MultiPeerTestObject* CreateAndRegisterObject(qcc::String name, qcc::String interface) {
        ...
        assert(ER_OK == bus.RegisterBusObject(*obj));
        return obj;
    }
...
class ObserverListener : public Observer::Listener {
  public:
    ...
    virtual void ObjectDiscovered(ManagedProxyBusObject& proxy) {
        ...
        assert(ER_OK == proxy->MethodCall(intfName.c_str(), METHOD_OBSUPDATER, updateArgs, 2)); // Tell the object that it's being observed by me
