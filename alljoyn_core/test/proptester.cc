...
static const char* const propTester2Names[] = { "int1", "int2", "int3", "string1", "string2", "string3" };
static const int propTester2Count = 6;
static bool multiProp = false;
    ...
    stop(false)
{
    ...
    for (int i = 0; i < propTester2Count; ++i) {
        propNames.push_back(propTester2Names[i]);
    }
...
ThreadReturn STDCALL PropTesterObject2::Run(void* arg)
{
    ...
    while (!IsStopping()) {
        ...
        EmitPropChanged("org.alljoyn.Testing.PropertyTester2", propNames, id);
    }
    lock.Unlock();
...
void Usage()
{
    printf("proptester: [ -c ] [ -n <NAME> ] [ -s <SECONDS> ]\n"
           ...
           "    -m            Use EmitPropertiesChanged for multiple properties at once.\n");
}
