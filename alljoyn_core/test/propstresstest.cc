...
void Client::LostAdvertisedName(const char* name, TransportMask transport, const char* namePrefix)
{
    ...
    foundNamesLock.Lock();
    map<String, SessionId>::iterator itNames = foundNames.find(nameStr);
    if (itNames != foundNames.end()) {
        QCC_SyncPrintf("LostAdvertisedName: \"%s\" (transport=%d)\n", name, transport);
        sessionId = itNames->second;
        foundNames.erase(itNames);
    } else {
        foundNamesLock.Unlock();
        return;
    }
    objectsLock.Lock();
    multimap<SessionId, PropTesterProxyObject>::iterator itObjects = objects.find(sessionId);
    while (itObjects != objects.end()) {
        itObjects->second->Unregister();
        objects.erase(itObjects);
        itObjects = objects.find(sessionId);
    }
    objectsLock.Unlock();
    foundNamesLock.Unlock();
}
...
void Client::JoinSessionCB(QStatus status, SessionId sessionId, const SessionOpts& opts, void* context)
{
    ...
    if (foundNames.find(*nameStr) == foundNames.end()) {
        foundNamesLock.Unlock();
        QCC_SyncPrintf("JoinSessionCB: Name \"%s\" already lost, exiting from callback\n", nameStr->c_str());
        delete nameStr;
        return;
    }
