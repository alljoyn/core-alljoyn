...
class PropTesterObject : public BusObject {
  public:
    ...
    void Set(int32_t int32Prop, uint32_t uint32Prop, const char* stringProp);
...
void PropTesterObject::Set(int32_t int32Prop, uint32_t uint32Prop, const char* stringProp)
{
    lock.Lock();
    ...
    lock.Unlock();
    EmitPropChanged(interfaceName, props, ArraySize(props), id, 0);
...
class App {
  public:
    virtual ~App() { }
    virtual void Execute(int timeToRun) = 0;
};
...
void Service::Execute(int timeToRun)
{
    time_t startTime = time(0);
    time_t stopTime = time(0);
    while ((timeToRun > difftime(stopTime, startTime)) && !quit) {
        multimap<SessionId, PropTesterObject*>::iterator it;
        int32_t int32 = 0;
        uint32_t uint32 = 0;
        String string = "Test";
...
void Client::Execute(int timeToRun)
{
    time_t startTime = time(0);
    time_t stopTime = time(0);
    bool seed = false;
