    ...
    while (ER_OK == status) {
        ...
        if (cmd == "debug") {
            ...
            if (namePrefix.empty()) {
                ...
                printf("Example: find com.yadda TRANSPORT_TCP\n");
                continue;
            }
            ...
            printf("find <name_prefix> [<transport_mask_enum>]                    - Discover names that begin with prefix on an specific transport. Transports can be TRANSPORT_(TCP,UDP,LOCAL or ANY)\n");
            printf("cancelfind <name_prefix> [<transport_mask_enum>]              - Cancel discovering names that begins with prefix on an specific transport. Transports can be TRANSPORT_(TCP,UDP,LOCAL or ANY)\n");
