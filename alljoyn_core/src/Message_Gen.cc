                            ...
                            uint16_t timeToLive)
{
    ...
    /*
     * Validate flags - ENCRYPTED, COMPRESSED, ALLJOYN_FLAG_GLOBAL_BROADCAST, ALLJOYN_FLAG_ALLSESSION_BROADCAST and ALLJOYN_FLAG_SESSIONLESS are the flags applicable to signals
     */
    if (flags & ~(ALLJOYN_FLAG_ENCRYPTED | ALLJOYN_FLAG_COMPRESSED | ALLJOYN_FLAG_GLOBAL_BROADCAST | ALLJOYN_FLAG_ALLSESSION_BROADCAST | ALLJOYN_FLAG_SESSIONLESS)) {
        return ER_BUS_BAD_HDR_FLAGS;
    }
    /*
     * Clear any stale header fields
