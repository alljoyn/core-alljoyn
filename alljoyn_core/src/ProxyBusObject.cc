...
class CachedProps {
    qcc::Mutex lock;
    typedef std::map<qcc::String, MsgArg> ValueMap;
    ValueMap values;
  ...
  public:
    ...
    CachedProps(const InterfaceDescription*intf) : lock(), values(), description(intf) {
        ...
        const InterfaceDescription::Property* props[numProperties];
...
struct ProxyBusObject::Components {
    ...
    map<qcc::String, int> matchRuleRefcounts;
                                                          ...
                                                          void* context)
{
    ...
    if (!replace) {
        ...
        AddPropertiesChangedRule(ifaceStr);
    }
                                                            ...
                                                            ProxyBusObject::PropertiesChangedListener& listener)
{
    ...
    QStatus status = ER_OK;
    if (removed) {
        RemovePropertiesChangedRule(ifaceStr);
    }
...
void ProxyBusObject::AddPropertiesChangedRule(const qcc::String& intf)
{
    lock->Lock(MUTEX_CONTEXT);
    ...
    std::map<qcc::String, int>::iterator it = components->matchRuleRefcounts.find(intf);
    if (it == components->matchRuleRefcounts.end()) {
        String rule("type='signal',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged',arg0='" + intf + "'");
        bus->AddMatchNonBlocking(rule.c_str());
        components->matchRuleRefcounts[intf] = 1;
    } else {
        it->second++;
    }
    lock->Unlock(MUTEX_CONTEXT);
...
void ProxyBusObject::RemovePropertiesChangedRule(const qcc::String& intf)
{
    lock->Lock(MUTEX_CONTEXT);
    std::map<qcc::String, int>::iterator it = components->matchRuleRefcounts.find(intf);
    if (it != components->matchRuleRefcounts.end()) {
...
{
    lock->Lock(MUTEX_CONTEXT);
    std::map<qcc::String, int>::iterator it = components->matchRuleRefcounts.begin();
    for (; it != components->matchRuleRefcounts.end(); ++it) {
        String rule("type='signal',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged',arg0='" + it->first + "'");
        bus->RemoveMatchNonBlocking(rule.c_str());
    }
