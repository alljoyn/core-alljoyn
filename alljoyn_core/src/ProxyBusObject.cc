...
class CachedProps {
  ...
  public:
    CachedProps() : lock(), values(), description(NULL) { }
    CachedProps(const InterfaceDescription*intf) : lock(), values(), description(intf) { }
    CachedProps(const CachedProps& other) : lock(), values(other.values), description(other.description) { }
    ...
    if (!cacheProperties) {
        ...
        for (; it != components->ifaces.end(); ++it) {
            if (it->second->HasCacheableProperties()) {
                ...
                /* add match rules in case the PropertiesChanged signals are emitted as global broadcast */
                String rule = qcc::String("type='signal',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged',arg0='") + it->first.c_str() + "'";
                bus->AddMatchNonBlocking(rule.c_str());
            }
        }
    }
...
void ProxyBusObject::DestructComponents()
{
    ...
    if (lock && components) {
        ...
        for (; cit != components->caches.end(); ++cit) {
            String rule = qcc::String("type='signal',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged',arg0='") + cit->first.c_str() + "'";
...
ProxyBusObject::ProxyBusObject(const ProxyBusObject& other) :
    ...
    cacheProperties(other.cacheProperties)
{
