...
QStatus PermissionConfigurator::ComputeThumbprintAndSignManifestXml(const qcc::CertificateX509& subjectCertificate, AJ_PCSTR* manifestXml)
{
    QCC_DbgTrace(("%s", __FUNCTION__));
    ...
    std::vector<PermissionPolicy::Rule> rules;
    QStatus status = XmlManifestTemplateConverter::GetInstance()->XmlToRules(*manifestXml, rules);
    if (status != ER_OK) {
    ...
    status = XmlManifestConverter::ManifestToXml(manifest, strManifestXml);
    if (status != ER_OK) {
        QCC_LogError(status, ("Could not convert manifest to string"));
        return status;
    }

    *manifestXml = strManifestXml.c_str();
    return status;
}
