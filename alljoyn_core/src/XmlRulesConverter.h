...
class XmlRulesConverter {
  ...
  private:
    ...
    /*
     * Map used to extract PermissionPolicy::Rule::Member::MemberType enum from XML contents.
     */
    static const std::unordered_map<std::string, PermissionPolicy::Rule::Member::MemberType> s_memberTypeMap;

    /*
     * Map used to extract XML member type values from the PermissionPolicy::Rule::Member::MemberType enum.
     */
    static const std::unordered_map<PermissionPolicy::Rule::Member::MemberType, std::string> s_inverseMemberTypeMap;

    /*
     * Map used to extract action masks from XML contents.
     */
    static const std::unordered_map<std::string, uint8_t> s_memberMasksMap;

    /*
     * Validator to determine if the parsed XML follows the rules XML schema.
     */
    static const XmlRulesValidator s_validator;
