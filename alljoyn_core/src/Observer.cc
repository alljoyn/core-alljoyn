...
Observer::Observer(BusAttachment& bus,
                   ...
                   const char* optionalInterfaces[],
                   size_t numOptionalInterfaces)
{
    set<qcc::String> mandatory;
    set<qcc::String> optional;
    bool inError = false;
    ...
    if (optionalInterfaces) {
        for (size_t i = 0; i < numOptionalInterfaces; ++i) {
            const InterfaceDescription* intf = bus.GetInterface(optionalInterfaces[i]);
            if (NULL == intf) {
                QCC_LogError(ER_FAIL, ("Interface %s does not exist", optionalInterfaces[i]));
                inError = true;
            } else {
                optional.insert(optionalInterfaces[i]);
            }
        }
    }
    ...
    if (inError) {
        ...
        internal = new Internal(bus, this, mandatory, optional);
    }
}
