...
const std::unordered_map<std::string, PermissionPolicy::Rule::Member::MemberType> XmlRulesConverter::s_memberTypeMap =
...
const std::unordered_map<PermissionPolicy::Rule::Member::MemberType, std::string> XmlRulesConverter::s_inverseMemberTypeMap =
    std::unordered_map<PermissionPolicy::Rule::Member::MemberType, std::string>({
        { PermissionPolicy::Rule::Member::MemberType::METHOD_CALL, METHOD_MEMBER_TYPE },
        { PermissionPolicy::Rule::Member::MemberType::PROPERTY, PROPERTY_MEMBER_TYPE },
        { PermissionPolicy::Rule::Member::MemberType::SIGNAL, SIGNAL_MEMBER_TYPE }
    });

const std::unordered_map<std::string, uint8_t> XmlRulesConverter::s_memberMasksMap =
...
const XmlRulesValidator XmlRulesConverter::s_validator;
...
QStatus XmlRulesConverter::XmlToRules(AJ_PCSTR rulesXml, std::vector<PermissionPolicy::Rule>& rules)
{
    ...
    if (ER_OK == status) {
        status = s_validator.Validate(root);
    }
...
QStatus XmlRulesConverter::RulesToXml(const PermissionPolicy::Rule* rules, const size_t rulesCount, AJ_PSTR* rulesXml)
{
    ...
    QStatus status = s_validator.ValidateRules(rules, rulesCount);
...
void XmlRulesConverter::SetMemberType(const qcc::XmlElement* xmlMember, PermissionPolicy::Rule::Member& member)
{
    ...
    member.SetMemberType(s_memberTypeMap.find(type)->second);
}
