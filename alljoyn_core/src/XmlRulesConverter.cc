...
{
    QCC_DbgTrace(("%s: Building rules.", __FUNCTION__));
...
{
    QCC_DbgTrace(("%s: Adding rules for each interface.", __FUNCTION__));
...
{
    QCC_DbgTrace(("%s: Adding a single rule for object path: %s.",
...
void XmlRulesConverter::SetInterfaceName(const XmlElement* singleInterface, PermissionPolicy::Rule& rule)
{
    ...
    QCC_DbgTrace(("%s: Setting interface name: %s.",
...
{
    QCC_DbgTrace(("%s: Adding rule members.", __FUNCTION__));
...
void XmlRulesConverter::UpdateRuleSecurityLevel(const map<string, string>& annotationsMap, PermissionPolicy::Rule& rule)
{
    ...
    if (nameToValuePair != annotationsMap.end()) {
        QCC_DbgTrace(("%s: Setting rule's security level to \"%s\".",
...
void XmlRulesConverter::SetMemberName(const XmlElement* xmlMember, PermissionPolicy::Rule::Member& member)
{
    ...
    QCC_DbgTrace(("%s: Setting member's name to \"%s\".",
...
void XmlRulesConverter::SetMemberType(const XmlElement* xmlMember, PermissionPolicy::Rule::Member& member)
{
    ...
    QCC_DbgTrace(("%s: Setting member's type to \"%s\".",
                  __FUNCTION__, type));
...
void XmlRulesConverter::SetMemberMask(const XmlElement* xmlMember, PermissionPolicy::Rule::Member& member)
{
    ...
    QCC_DbgTrace(("%s: Setting member's mask to %hhx.",
                  __FUNCTION__, mask));
...
{
    QCC_DbgTrace(("%s: Building XML \"" NODE_XML_ELEMENT "\" element.", __FUNCTION__));
...
{
    QCC_DbgTrace(("%s: Building XML \"" INTERFACE_XML_ELEMENT "\" element.", __FUNCTION__));
...
void XmlRulesConverter::BuildXmlMember(const PermissionPolicy::Rule::Member& member, XmlElement* interfaceElement)
{
    ...
    QCC_DbgTrace(("%s: Building XML \"%s\" element.",
...
{
    QCC_DbgTrace(("%s: Adding an \"" ANNOTATION_XML_ELEMENT "\" element with name=\"%s\" and value=\"%s\".",
