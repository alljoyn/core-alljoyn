...
const size_t Crypto::NonceLength = 14;
const size_t Crypto::PreviousNonceLength = 5;

const size_t Crypto::MaxNonceLength = 14;

size_t Crypto::GetMACLength(const _Message& message) {
    uint32_t authV = message.GetAuthVersion() >> 16;
    size_t macLen = MACLength;

    if (_Message::MIN_AUTH_VERSION_MACLEN16 > authV) {
...
size_t Crypto::GetNonceLength(const _Message& message) {
    ...
    if (_Message::MIN_AUTH_VERSION_FULLNONCELEN > authV) {
 ...
 *
 *
 * When the Auth Version is greater than or equal to 3, the Nonce is 14 bytes and has the following layout:
 *
 *  Nonce:
 *      byte0       -   Byte indicating role.
 *      bytes1...4  -   Big Endian Serial Number (32bits/4bytes.)
 *      byte5       -   Reserved 0.
 *      bytes6...13 -   Big Endian Crypto Random Value (64bits/8bytes.)
    ...
    case KeyBlob::AES:
        {
            ...
            if (message.RequiresCryptoValue()) {
                ...
                nd[13] = (uint8_t)randomCryptoValue;
                randomCryptoValue >>= 8;
