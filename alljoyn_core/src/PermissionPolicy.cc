...
static void RulesToManifestTemplate(const PermissionPolicy::Rule* rules, size_t count, vector<PermissionPolicy::Rule>& manifestTemplate)
{
    manifestTemplate.assign(rules, rules + count);
    for (auto& manifestTemplateRule : manifestTemplate) {
        manifestTemplateRule.SetRuleType(PermissionPolicy::Rule::MANIFEST_TEMPLATE_RULE);
    }
}
...
QStatus PermissionPolicy::ManifestTemplateToMsgArg(const Rule* rules, size_t count, MsgArg& msgArg)
{
    ...
    RulesToManifestTemplate(rules, count, manifestTemplate);
    status = GenerateRuleArgs(&rulesArgs, manifestTemplate.data(), count);
    if (ER_OK != status) {
        return status;
    }
