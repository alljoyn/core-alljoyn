...
QStatus XmlHelper::ParseInterface(const XmlElement* elem, ProxyBusObject* obj)
{
    ...
    while ((ER_OK == status) && (ifIt != elem->GetChildren().end())) {
        ...
        if ((ifChildName == "method") || (ifChildName == "signal")) {
            if (IsLegalMemberName(memberName.c_str())) {
                ...
                while ((ER_OK == status) && (argIt != argChildren.end())) {
                    ...
                    if (argElem->GetName() == "arg") {
                        ...
                        if (!nameAtt.empty()) {
                            ...
                            for (; it != argElem->GetChildren().end(); it++) {
                                if ((*it)->GetName() == "annotation") {
                                    ...
                                    if ((*it)->GetAttribute("name").compare(0, 25, "org.alljoyn.Bus.DocString") == 0) {
                                    ...
                                    if (!lang.empty()) {
                                        const pair<qcc::String, qcc::String> item(nameAtt, qcc::String("org.alljoyn.Bus.DocString.") + lang);
                        ...
                        if (!lang.empty()) {
                            const qcc::String nameAtt = "org.alljoyn.Bus.DocString." + lang;
            ...
            if (!SignatureUtils::IsCompleteType(sig.c_str())) {
                ...
                while ((ER_OK == status) && (argIt != argChildren.end())) {
                    ...
                    if (argElem->GetName() == "annotation") {
                        if (argElem->GetAttribute("name").compare(0, 25, "org.alljoyn.Bus.DocString") == 0) {
                        ...
                        const qcc::String& lang = argElem->GetAttribute("language");
                        if (!lang.empty()) {
                            intf.AddPropertyAnnotation(memberName, qcc::String("org.alljoyn.Bus.DocString.") + lang, content);
                        }
            ...
            if (!language.empty()) {
                const qcc::String nameAtt = "org.alljoyn.Bus.DocString." + language;
