...
const char* StringTableTranslator::Translate(const char* sourceLanguage, const char* targetLanguage, const char* source)
{
    QCC_UNUSED(sourceLanguage);
    return localizedStore[source][targetLanguage].c_str();
}

const char* MsgArgTableTranslator::Translate(const char* sourceLanguage, const char* targetLanguage, const char* source)
{
    QCC_UNUSED(sourceLanguage);
    char* result;
    QStatus status = localizedStore[source][targetLanguage].Get("s", &result);
    return (status == ER_OK) ? result : nullptr;
}

QStatus MsgArgTableTranslator::TranslateToMsgArg(const char* sourceLanguage, const char* targetLanguage, const char* source, ajn::MsgArg*& value)
{
    QCC_UNUSED(sourceLanguage);
    value = &localizedStore[source][targetLanguage];
    return ER_OK;
