...
const std::unordered_map<std::string, PermissionPolicy::Rule::Member::MemberType> XmlRulesValidator::MemberValidator::s_memberTypeMap =
    std::unordered_map<std::string, PermissionPolicy::Rule::Member::MemberType>({
        { METHOD_MEMBER_TYPE, PermissionPolicy::Rule::Member::MemberType::METHOD_CALL },
        { PROPERTY_MEMBER_TYPE, PermissionPolicy::Rule::Member::MemberType::PROPERTY },
        { SIGNAL_MEMBER_TYPE, PermissionPolicy::Rule::Member::MemberType::SIGNAL }
    });

const std::unordered_map<std::string, uint8_t> XmlRulesValidator::MethodsValidator::s_actionsMap =
    std::unordered_map<std::string, uint8_t>({
        { MODIFY_MEMBER_MASK, PermissionPolicy::Rule::Member::ACTION_MODIFY },
        { PROVIDE_MEMBER_MASK, PermissionPolicy::Rule::Member::ACTION_PROVIDE }
    });

const std::unordered_map<std::string, uint8_t> XmlRulesValidator::PropertiesValidator::s_actionsMap =
    std::unordered_map<std::string, uint8_t>({
        { MODIFY_MEMBER_MASK, PermissionPolicy::Rule::Member::ACTION_MODIFY },
        { PROVIDE_MEMBER_MASK, PermissionPolicy::Rule::Member::ACTION_PROVIDE },
        { OBSERVE_MEMBER_MASK, PermissionPolicy::Rule::Member::ACTION_OBSERVE }
    });

const std::unordered_map<std::string, uint8_t> XmlRulesValidator::SignalsValidator::s_actionsMap =
    std::unordered_map<std::string, uint8_t>({
        { PROVIDE_MEMBER_MASK, PermissionPolicy::Rule::Member::ACTION_PROVIDE },
        { OBSERVE_MEMBER_MASK, PermissionPolicy::Rule::Member::ACTION_OBSERVE }
    });
...
QStatus XmlRulesValidator::Validate(const qcc::XmlElement* rootElement) const
{
    QCC_ASSERT(nullptr != rootElement);

    std::unordered_set<std::string> nodeNames;
    QStatus status = XmlValidator::ValidateChildrenCountPositive(rootElement);
...
QStatus XmlRulesValidator::ValidateRules(const PermissionPolicy::Rule* rules, const size_t rulesCount) const
{
...
{
    QStatus status = ValidateString(rule.GetObjPath().c_str(), s_objectPathRegex);
...
{
    QStatus status = XmlValidator::ValidateElementName(node, NODE_XML_ELEMENT);
...
{
    std::unordered_set<std::string> interfaceNames;
    QStatus status = XmlValidator::ValidateChildrenCountPositive(node);
...
{
    QStatus status = XmlValidator::ValidateElementName(singleInterface, INTERFACE_XML_ELEMENT);
...
{
    MemberValidatorFactory memberValidatorFactory;
    QStatus status = XmlValidator::ValidateChildrenCountPositive(singleInterface);
...
QStatus XmlRulesValidator::MemberValidator::ValidateAnnotations(const qcc::XmlElement* member)
{
    ...
    QStatus status = XmlValidator::ValidateChildrenCountPositive(member);
...
{
    QStatus status = XmlValidator::ValidateElementName(annotation, ANNOTATION_XML_ELEMENT);
