...
class PermissionMgmtObj : public BusObject {
  ...
  public:
    ...
    class TrustAnchorList {
      public:
        typedef std::vector<std::shared_ptr<TrustAnchor> >::iterator iterator;
        typedef std::vector<std::shared_ptr<TrustAnchor> >::const_iterator const_iterator;

        iterator begin() { return anchors.begin(); }
        const_iterator begin() const { return anchors.begin(); }
        iterator end() { return anchors.end(); }
        const_iterator end() const { return anchors.end(); }

        void push_back(std::shared_ptr<TrustAnchor> ta) { anchors.push_back(ta); }
        void clear() { anchors.clear(); }
        bool empty() const { return anchors.empty(); }
        std::vector<std::shared_ptr<TrustAnchor> >::size_type size() const { return anchors.size(); }

        QStatus Lock(const char* file, uint32_t line) { return lock.Lock(file, line); }
        QStatus Unlock(const char* file, uint32_t line) { return lock.Unlock(file, line); }
        QStatus Lock() { return lock.Lock(); }
        QStatus Unlock() { return lock.Unlock(); }

      private:
        std::vector<std::shared_ptr<TrustAnchor> > anchors;
        qcc::Mutex lock;
    };
