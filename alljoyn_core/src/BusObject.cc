        ...
        if (!msg->IsEncrypted() && SecurityApplies(this, ifc)) {
            ...
            if (prop) {
                if (prop->access & PROP_ACCESS_READ) {
                    status = Get(iface->v_string.str, property->v_string.str, val, errorName.c_str(), errorMessage.c_str());
                } else {
        ...
        if (!msg->IsEncrypted() && SecurityApplies(this, ifc)) {
            ...
            if (prop) {
                if (!val->v_variant.val->HasSignature(prop->signature.c_str())) {
                    ...
                    status = Set(iface->v_string.str, property->v_string.str, *(val->v_variant.val), errorName.c_str(), errorMessage.c_str());
                } else {
