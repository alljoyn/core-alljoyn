                          ...
                          Message* outMsg)
{
    ...
    if (signalMember.isSessioncastSignal ||
        ...
        if ((sessionId != 0) && !signalMember.isSessioncastSignal) {
            QCC_LogError(ER_FAIL, ("Attempt to send a sessioncast signal when %s is not sessioncast", signalMember.name.c_str()));
            return ER_FAIL;
        }
        if ((flags & ALLJOYN_FLAG_SESSIONLESS) && !signalMember.isSessionlessSignal) {
            QCC_LogError(ER_FAIL, ("Attempt to send a sessionless signal when %s is not sessionless", signalMember.name.c_str()));
            return ER_FAIL;
        }
        if ((destination != NULL) && !signalMember.isUnicastSignal) {
            QCC_LogError(ER_FAIL, ("Attempt to send a unicast signal when %s is not unicast", signalMember.name.c_str()));
            return ER_FAIL;
        }
        if ((flags & ALLJOYN_FLAG_GLOBAL_BROADCAST) && !signalMember.isGlobalBroadcastSignal) {
            QCC_LogError(ER_FAIL, ("Attempt to send a global broadcast signal when %s is not global broadcast", signalMember.name.c_str()));
            return ER_FAIL;
        }
    }
