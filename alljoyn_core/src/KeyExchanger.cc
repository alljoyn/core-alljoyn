...
QStatus KeyExchangerECDHE::RespondToKeyExchange(Message& msg, MsgArg* variant, uint32_t remoteAuthMask, uint32_t authMask)
{
    ...
    peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_KEYEXCHANGEREQUEST);
    ...
    peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_KEYEXCHANGEREPLY);
...
QStatus KeyExchangerECDHE::ExecKeyExchange(uint32_t authMask, KeyExchangerCB& callback, uint32_t* remoteAuthMask)
{
    ...
    /* Hash the handshake data for version 4. */
    peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_KEYEXCHANGEREQUEST);
    peerState->UpdateHash(CONVERSATION_V4, args, ArraySize(args));
...
QStatus KeyExchanger::ValidateRemoteVerifierVariant(const char* peerName, MsgArg* variant, uint8_t* authorized)
{
    ...
    *authorized = (Crypto_Compare(remoteVerifier, computedRemoteVerifier, sizeof(computedRemoteVerifier)) == 0);
    if (!IsInitiator()) {
        ...
        peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_VERIFIER);
        peerState->UpdateHash(CONVERSATION_V4, *variant);
    }
    return ER_OK;
}
...
QStatus KeyExchangerECDHE_NULL::KeyAuthentication(KeyExchangerCB& callback, const char* peerName, uint8_t* authorized)
{
    *authorized = false;
    ...
    peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_VERIFIER);
    peerState->UpdateHash(CONVERSATION_V4, verifierArg);
...
QStatus KeyExchangerECDHE_PSK::ValidateRemoteVerifierVariant(const char* peerName, MsgArg* variant, uint8_t* authorized)
{
    ...
    *authorized = false;
    ...
    if (!IsInitiator()) {
        ...
        peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_PSK);
    ...
    *authorized = (Crypto_Compare(remoteVerifier, computedRemoteVerifier, sizeof(computedRemoteVerifier)) == 0);
    if (!IsInitiator()) {
        ...
        peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_VERIFIER);
        peerState->UpdateHash(CONVERSATION_V4, remoteVerifier, remoteVerifierLen);
    }
    return ER_OK;
}
...
QStatus KeyExchangerECDHE_PSK::KeyAuthentication(KeyExchangerCB& callback, const char* peerName, uint8_t* authorized)
{
    *authorized = false;
    ...
    peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_VERIFIER);
    peerState->UpdateHash(CONVERSATION_V4, verifier, sizeof(verifier));
...
QStatus KeyExchangerECDHE_ECDSA::ValidateRemoteVerifierVariant(const char* peerName, MsgArg* variant, uint8_t* authorized)
{
    ...
    *authorized = false;
    ...
    /* Hashing the variant for CONVERSATION_V4 covers the individual CONVERSATION_V1 hashing
     * done below.
     */
    peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_ECDSA);
    peerState->UpdateHash(CONVERSATION_V4, *variant);
...
QStatus KeyExchangerECDHE_ECDSA::GenVerifierSigInfoArg(MsgArg& msgArg, bool updateHash)
{
    ...
    if (updateHash) {
        ...
        /* Hashing for CONVERSATION_V4 is handled later. */
    }
    ...
    if (updateHash) {
        peerState->UpdateHash(CONVERSATION_V4, HASH_HEADER_ECDSA);
        /* Hashing msgArg will incorporate all of the content done individually in CONVERSATION_V1 above. */
        peerState->UpdateHash(CONVERSATION_V4, msgArg);
    }
