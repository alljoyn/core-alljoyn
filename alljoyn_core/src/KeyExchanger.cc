...
void KeyExchanger::ShowCurrentDigest(const char* ref)
{
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.GetDigest(digest, true);
...
QStatus KeyExchangerECDHE::RespondToKeyExchange(Message& msg, MsgArg* variant, uint32_t remoteAuthMask, uint32_t authMask)
{
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, HexStringToByteString(U32ToString(remoteAuthMask, 16, 2 * sizeof(remoteAuthMask), '0')));
    ...
    /* hash the handshake data */
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, HexStringToByteString(U32ToString(authMask, 16, 2 * sizeof(authMask), '0')));
    conversation.ReleaseConversationHashLock();
...
void KeyExchangerECDHE::KeyExchangeGenLegacyKey(MsgArg& variant)
{
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, buf, sizeof(buf));
    conversation.ReleaseConversationHashLock();
...
QStatus KeyExchangerECDHE::KeyExchangeReadLegacyKey(MsgArg& variant)
{
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, replyPubKey, replyPubKeyLen);
    conversation.ReleaseConversationHashLock();
...
void KeyExchangerECDHE::KeyExchangeGenKeyInfo(MsgArg& variant)
{
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, exportedPublicKey, exportedPublicKeySize);
    conversation.ReleaseConversationHashLock();
...
void KeyExchangerECDHE::KeyExchangeGenKey(MsgArg& variant)
{
    if (PeerSupportsKeyInfo()) {
        ...
        _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
        conversation.AcquireConversationHashLock();
        conversation.UpdateHash(CONVERSATION_V1, &curveType, sizeof(curveType));
        conversation.UpdateHash(CONVERSATION_V1, exportedPublicKey, exportedPublicKeySize);
        conversation.ReleaseConversationHashLock();
...
QStatus KeyExchangerECDHE::KeyExchangeReadKey(MsgArg& variant)
{
    if (!PeerSupportsKeyInfo()) {
        ...
        _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
        conversation.AcquireConversationHashLock();
        conversation.UpdateHash(CONVERSATION_V1, &eccCurveID, sizeof(eccCurveID));
        conversation.UpdateHash(CONVERSATION_V1, replyPubKey, replyPubKeyLen);
        conversation.ReleaseConversationHashLock();
...
QStatus KeyExchangerECDHE::ExecKeyExchange(uint32_t authMask, KeyExchangerCB& callback, uint32_t* remoteAuthMask)
{
    ...
    conversation.UpdateHash(CONVERSATION_V4, sentMsg);
    conversation.UpdateHash(CONVERSATION_V4, replyMsg);
    conversation.ReleaseConversationHashLock();
    if (status != ER_OK) {
...
QStatus KeyExchanger::ValidateRemoteVerifierVariant(const char* peerName, MsgArg* variant, uint8_t* authorized)
{
    ...
    *authorized = (Crypto_Compare(remoteVerifier, computedRemoteVerifier, sizeof(computedRemoteVerifier)) == 0);
    if (!IsInitiator()) {
        _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
        conversation.AcquireConversationHashLock();
        conversation.UpdateHash(CONVERSATION_V1, remoteVerifier, remoteVerifierLen);
        conversation.ReleaseConversationHashLock();
...
QStatus KeyExchangerECDHE_NULL::KeyAuthentication(KeyExchangerCB& callback, const char* peerName, uint8_t* authorized)
{
    *authorized = false;
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, verifier, sizeof(verifier));
...
QStatus KeyExchangerECDHE_PSK::ValidateRemoteVerifierVariant(const char* peerName, MsgArg* variant, uint8_t* authorized)
{
    ...
    *authorized = false;
    ...
    if (!IsInitiator()) {
        ...
        _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
        conversation.AcquireConversationHashLock();
        conversation.UpdateHash(CONVERSATION_V1, peerPskName, peerPskNameLen);
         ...
         */
        conversation.SetConversationHashSensitiveMode(true);
        conversation.UpdateHash(CONVERSATION_V1, (const uint8_t*) pskValue.data(), pskValue.length());
        conversation.SetConversationHashSensitiveMode(false);
        conversation.ReleaseConversationHashLock();
    }
    if (remoteVerifierLen != AUTH_VERIFIER_LEN) {
        return ER_INVALID_DATA;
...
QStatus KeyExchangerECDHE_PSK::KeyAuthentication(KeyExchangerCB& callback, const char* peerName, uint8_t* authorized)
{
    *authorized = false;
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, (const uint8_t*)pskName.data(), pskName.length());
    /* Calling SetConversationHashSensitiveMode ensures the PSK won't end up in the log if conversation
     * hash tracing is turned on.
     */
    conversation.SetConversationHashSensitiveMode(true);
...
QStatus KeyExchangerECDHE_ECDSA::ValidateRemoteVerifierVariant(const char* peerName, MsgArg* variant, uint8_t* authorized)
{
    ...
    _PeerState::Conversation& conversation = peerState->GetConversation(IsInitiator());
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, rCoord, rCoordLen);
    conversation.UpdateHash(CONVERSATION_V1, sCoord, sCoordLen);
    conversation.ReleaseConversationHashLock();
    ...
    conversation.AcquireConversationHashLock();
    conversation.UpdateHash(CONVERSATION_V1, &certChainEncoding, sizeof(certChainEncoding));
    conversation.ReleaseConversationHashLock();
    if (numCerts == 0) {
    ...
    for (size_t cnt = 0; cnt < numCerts; cnt++) {
        ...
        conversation.AcquireConversationHashLock();
        conversation.UpdateHash(CONVERSATION_V1, encoded, encodedLen);
        conversation.ReleaseConversationHashLock();
    }
    /* verify signature */
...
QStatus KeyExchangerECDHE_ECDSA::GenVerifierSigInfoArg(MsgArg& msgArg, bool updateHash)
{
    ...
    if (updateHash) {
        conversation.AcquireConversationHashLock();
        conversation.UpdateHash(CONVERSATION_V1, sigInfo.GetRCoord(), sigInfo.GetRSize());
        conversation.UpdateHash(CONVERSATION_V1, sigInfo.GetSCoord(), sigInfo.GetSSize());
        conversation.ReleaseConversationHashLock();
    ...
    if (updateHash) {
        conversation.AcquireConversationHashLock();
        conversation.UpdateHash(CONVERSATION_V1, &encoding, sizeof(encoding));
        conversation.ReleaseConversationHashLock();
    }
    if (certChainLen > 0) {
        ...
        for (size_t cnt = 0; cnt < certChainLen; cnt++) {
            ...
            if (updateHash) {
                conversation.AcquireConversationHashLock();
                conversation.UpdateHash(CONVERSATION_V1, (const uint8_t*) der.data(), der.size());
                conversation.ReleaseConversationHashLock();
            }
        }
    }
