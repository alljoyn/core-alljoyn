...
BusAttachment::~BusAttachment(void)
{
    ...
    for (size_t side = 0; side < sizeof(busInternal->sessions) / sizeof(busInternal->sessions[0]); ++side) {
                                                   ...
                                                   Message& msg)
{
    ...
    if (msg->GetType() == MESSAGE_SIGNAL) {
        if (0 == strcmp("Announce", msg->GetMemberName())) {
            ...
            if (disposition < sizeof(sessions) / sizeof(sessions[0])) {
            ...
            for (size_t side = 0; side < sizeof(sessions) / sizeof(sessions[0]); ++side) {
