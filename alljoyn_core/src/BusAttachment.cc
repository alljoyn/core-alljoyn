                                                   ...
                                                   Message& msg)
{
    ...
    if (msg->GetType() == MESSAGE_SIGNAL) {
        if (0 == strcmp("FoundAdvertisedName", msg->GetMemberName())) {
            ...
            printf("SessionLostWithReason %u %u\n", id, reason);
            for (size_t i = 0; i < sizeof(sessionListeners) / sizeof(sessionListeners[0]); ++i) {
                ...
                if ((i == SESSION_SIDE_HOST &&
                     !(reason == SessionListener::SessionLostReason::ALLJOYN_SESSIONLOST_REMOVED_BY_BINDER_SELF ||
                       reason == SessionListener::SessionLostReason::ALLJOYN_SESSIONLOST_REMOVED_BY_BINDER)) ||
                    i == SESSION_SIDE_JOINER) {
            ...
            SessionId id = static_cast<SessionId>(args[0].v_uint32);
            MPSessionChangedReason reason = static_cast<MPSessionChangedReason>(args[3].v_uint32);
                ...
                if (slit != sessionListeners[i].end()) {
                    ...
                    if (*pl) {
                        if (args[2].v_bool) {
                            ...
                            if (i == SESSION_SIDE_JOINER || (i == SESSION_SIDE_HOST && (strcmp(member, bus.GetUniqueName().c_str()) == 0 || reason == ALLJOYN_MPSESSIONCHANGED_LOCAL_MEMBER_ADDED))) {
