...
class ProtectedAuthListener : public AuthListener {
  ...
  private:
    ...
    /**
     * Reference count used by the @Set method to wait until the inner @listener can be replaced.
     *
     * @remark Destroying the inner @listener from the application is incorrect and is not prevented by this reference count.
     */
