...
const std::unordered_map<std::string, PermissionPolicy::Peer::PeerType> XmlPoliciesValidator::PeerValidator::s_peerTypeMap =
    std::unordered_map<std::string, PermissionPolicy::Peer::PeerType>({
        { XML_PEER_ALL, PermissionPolicy::Peer::PeerType::PEER_ALL },
        { XML_PEER_ANY_TRUSTED, PermissionPolicy::Peer::PeerType::PEER_ANY_TRUSTED },
        { XML_PEER_FROM_CERTIFICATE_AUTHORITY, PermissionPolicy::Peer::PeerType::PEER_FROM_CERTIFICATE_AUTHORITY },
        { XML_PEER_WITH_MEMBERSHIP, PermissionPolicy::Peer::PeerType::PEER_WITH_MEMBERSHIP },
        { XML_PEER_WITH_PUBLIC_KEY, PermissionPolicy::Peer::PeerType::PEER_WITH_PUBLIC_KEY }
    });

const XmlRulesValidator XmlPoliciesValidator::s_rulesValidator;

QStatus XmlPoliciesValidator::Validate(const XmlElement* policyXml) const
{
    QStatus status = XmlValidator::ValidateElementName(policyXml, POLICY_XML_ELEMENT);

    if (ER_OK == status) {
        status = XmlValidator::ValidateChildrenCountEqual(policyXml, POLICIES_ROOT_ELEMENT_CHILDREN_COUNT);
    }
...
{
    QStatus status = XmlValidator::ValidateElementName(acls, ACLS_XML_ELEMENT);

    if (ER_OK == status) {
        status = XmlValidator::ValidateChildrenCountPositive(acls);
    }
...
{
    QStatus status = XmlValidator::ValidateElementName(acl, ACL_XML_ELEMENT);

    if (ER_OK == status) {
        status = XmlValidator::ValidateChildrenCountEqual(acl, ACL_ELEMENT_CHILDREN_COUNT);
    }
    ...
    if (ER_OK == status) {
        status = s_rulesValidator.Validate(acl->GetChildren()[RULES_INDEX]);
    }
...
QStatus XmlPoliciesValidator::ValidatePeers(const XmlElement* peers)
{
    ...
    QStatus status = XmlValidator::ValidateElementName(peers, PEERS_XML_ELEMENT);

    if (ER_OK == status) {
        status = XmlValidator::ValidateChildrenCountPositive(peers);
    }
...
QStatus XmlPoliciesValidator::ValidatePeer(const XmlElement* peer, PeerValidatorFactory& peerValidatorFactory)
{
    ...
    QStatus status = XmlValidator::ValidateChildrenCountPositive(peer);

    if (ER_OK == status) {
        status = XmlValidator::ValidateElementName(peer, PEER_XML_ELEMENT);
    }
...
QStatus XmlPoliciesValidator::WitheMembershipValidator::ValidateTypeSpecific(const qcc::XmlElement* peer)
{
...
QStatus XmlPoliciesValidator::WitheMembershipValidator::ValidateSgId(const qcc::XmlElement* peer)
{
    AJ_PCSTR sgId = peer->GetChildren()[PEER_SGID_INDEX]->GetContent().c_str();
    return qcc::GUID128::IsGUID(sgId) ? ER_OK : ER_XML_MALFORMED;
}

size_t XmlPoliciesValidator::WitheMembershipValidator::GetPeerChildrenCount()
{
    return PEER_WITH_MEMBERSHIP_ELEMENTS_COUNT;
}

std::string XmlPoliciesValidator::WitheMembershipValidator::GetPeerId(const qcc::XmlElement* peer)
{
