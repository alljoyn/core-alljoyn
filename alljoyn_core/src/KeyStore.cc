...
{
    QCC_VERIFY(ER_OK == lock.Lock(MUTEX_CONTEXT));
    ProtectedKeyStoreListener* oldListener = listener;
    QCC_VERIFY(ER_OK == lock.Unlock(MUTEX_CONTEXT));

    /**
     * We need to acquire the exclusive lock here to ensure that the listener
     * doesn't get pulled off while we're still holding the exlusive lock.
     * Calling release on the same listener is necessary here to ensure that
     * the exclusive lock is not orphaned.
     */
    QStatus status = oldListener->AcquireExclusiveLock(MUTEX_CONTEXT);
    if (status != ER_OK) {
        return status;
    }
    ProtectedKeyStoreListener* listenerHoldingExclusiveLock = oldListener;
    ...
    if (listenerHoldingExclusiveLock != nullptr) {
        listenerHoldingExclusiveLock->ReleaseExclusiveLock(MUTEX_CONTEXT);
    }

    /* It is now safe to delete the old listener outside all locks. */
    delete oldListener;
    return ER_OK;
}
