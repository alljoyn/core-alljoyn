...
class XmlPoliciesConverter {
  ...
  private:
    ...
    /*
     * Map used to extract PermissionPolicy::Peer::PeerType enum from XML contents.
     */
    static const std::unordered_map<std::string, PermissionPolicy::Peer::PeerType> s_peerTypeMap;

    /*
     * Validator to determine if the input XML is a valid policy XML.
     */
    static const XmlPoliciesValidator s_validator;
