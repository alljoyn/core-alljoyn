...
TEST(KeyStoreTest, concurrent_access_multiple_keystores)
{
    ...
    /* try resetting the default listener just to make sure doing so doesn't cause any problem */
    for (int i = 0; i < 100; ++i) {
        uint32_t rnd = qcc::Rand32();
        qcc::Sleep(rnd % 2);
        if ((rnd % 5) == 0) {
            thread1.GetKeyStore()->SetDefaultListener();
        }
        if ((rnd % 7) == 0) {
            thread2.GetKeyStore()->SetDefaultListener();
        }
    }
