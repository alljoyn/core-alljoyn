...
#define SLEEP_TIME_500 (500 * GlobalTimerMultiplier)
#define SLEEP_TIME_1000 (1000 * GlobalTimerMultiplier)
#define SLEEP_TIME_2000 (2000 * GlobalTimerMultiplier)
#define TIMEOUT (10000 * GlobalTimerMultiplier)
...
/* This is a blocking test. The idea is to send out 12 signals, the first signal handler
   will sleep for SLEEP_TIME_2000, as a result of which the SendSignal should block for approx
   SLEEP_TIME_2000 ms until that signal handler returns.
...
TEST_F(SignalTest, BackPressure) {
    ...
    if (A.inited && B.inited) {
        ...
        EXPECT_GE(elapsed, SLEEP_TIME_2000 - QCC_TIMESTAMP_GRANULARITY);
        wait_for_signal();
...
class SecSignalTest :
  ...
  public:
    ...
    QStatus SendAndWaitForEvent(TestSecureApplication& prov, bool newValue = true, TestSecureApplication* destination = NULL, int requiredEvents = 1)
    {
        ...
        if (status == ER_OK) {
            EXPECT_EQ(ER_OK, (status = condition.TimedWait(lock, TIMEOUT)));
