...
TEST_F(SecSignalTest, SecureConnection)
{
    ...
    EXPECT_EQ(ER_OK, prov.SecureConnection(cons));
    ...
    // ASACORE-2432 explains that the active sessions must be secured again - e.g. from the EndManagement callback.
    ...
    EXPECT_EQ(ER_OK, cons2.SecureConnection(prov));
    ...
    EXPECT_EQ(ER_OK, prov.SetAnyTrustedUserPolicy(tsm, PermissionPolicy::Rule::Member::ACTION_OBSERVE | PermissionPolicy::Rule::Member::ACTION_MODIFY));
    EXPECT_EQ(ER_OK, prov.SecureConnectionAllSessions(cons, cons2));
    EXPECT_EQ(ER_OK, SendAndWaitForEvent(prov, true, NULL, 3));
    ...
    // Secure connections from the consumer side --> events will be sent to this consumer.
    EXPECT_EQ(ER_OK, cons.SecureConnectionAllSessions(prov));
    ...
    EXPECT_EQ(ER_OK, prov.SetAnyTrustedUserPolicy(tsm, PermissionPolicy::Rule::Member::ACTION_OBSERVE));
    EXPECT_EQ(ER_OK, prov.SecureConnectionAllSessionsAsync(cons, cons2));
    EXPECT_EQ(ER_OK, SendAndWaitForEvent(prov, true, NULL, 3));
    ...
    EXPECT_EQ(ER_OK, cons.SecureConnectionAllSessionsAsync(prov));
    EXPECT_EQ(ER_OK, SendAndWaitForEvent(prov, true, NULL, 3)); // All events should be received.
}
