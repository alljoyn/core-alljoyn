...
class V1ECDHEKeyXListener : public AuthListener {
  public:
    V1ECDHEKeyXListener(BusAttachment& caBus) : caBus(caBus), ecc()
    {
        ...
        PermissionMgmtTestHelper::CreateIdentityCert(caBus, "9010101", "V1Client", ecc.GetDSAPublicKey(), "V1 Client", 3600, identityCert, digest, Crypto_SHA256::DIGEST_SIZE);
    }
    ...
    bool RequestCredentials(const char* authMechanism, const char* authPeer, uint16_t authCount, const char* userId, uint16_t credMask, Credentials& creds)
    {
        ...
        if (strcmp(authMechanism, "ALLJOYN_ECDHE_ECDSA") == 0) {
            if ((credMask& AuthListener::CRED_PRIVATE_KEY) == AuthListener::CRED_PRIVATE_KEY) {
                String pk;
                CertificateX509::EncodePrivateKeyPEM(ecc.GetDSAPrivateKey(), pk);
                creds.SetPrivateKey(pk);
            }
            if ((credMask& AuthListener::CRED_CERT_CHAIN) == AuthListener::CRED_CERT_CHAIN) {
                String cert;
    ...
    bool VerifyCredentials(const char* authMechanism, const char* authPeer, const Credentials& creds)
    {
        ...
        if (strcmp(authMechanism, "ALLJOYN_ECDHE_ECDSA") == 0) {
            ...
            status = CertificateX509::DecodeCertChainPEM(creds.GetCertChain(), certChain, count);
            if (ER_OK != status) {
                delete [] certChain;
                return false;
            }
            ...
            caBus.GetPermissionConfigurator().GetSigningPublicKey(caKeyInfo);
 ...
 * able to send/received messages to/from Security 2.0 peer in a trusted manner,
...
TEST_F(SecurityOther2Test, security1_to_security2_over_ECDHE_NULL_acl_peer_ANY_TRUSTED) {
    ...
    SecurityOtherTestBusObject peer1BusObject(peer1Bus, "/test", interfaceName.c_str());
    EXPECT_EQ(ER_OK, peer1Bus.RegisterBusObject(peer1BusObject, true));

    SecurityOtherTestBusObject peer2BusObject(peer2Bus, "/test", interfaceName.c_str());
    EXPECT_EQ(ER_OK, peer2Bus.RegisterBusObject(peer2BusObject, true));
    ...
    ProxyBusObject proxy(peer1Bus, peer2Bus.GetUniqueName().c_str(), "/test", sessionId);
    EXPECT_EQ(ER_OK, proxy.ParseXml(interface.c_str()));
...
TEST_F(SecurityOther2Test, security2_to_security1_over_ECDHE_NULL_acl_peer_ANY_TRUSTED) {
    ...
    ProxyBusObject proxy(peer2Bus, peer1Bus.GetUniqueName().c_str(), "/test", sessionId);
    EXPECT_EQ(ER_OK, proxy.ParseXml(interface.c_str()));
    EXPECT_TRUE(proxy.ImplementsInterface(interfaceName.c_str())) << interface.c_str() << "\n";
    MsgArg arg("s", "String that should be Echoed back.");
