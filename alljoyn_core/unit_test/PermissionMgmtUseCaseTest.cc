...
class PermissionMgmtUseCaseTest : public BasePermissionMgmtTest {
  ...
  public:
    ...
    void Install3CertMembershipChainToTarget(BusAttachment& caBus, BusAttachment& middleBus, BusAttachment& targetBus, qcc::String leafSerial, qcc::GUID128& sgID)
    {
        ...
        EXPECT_EQ(ER_OK, saProxy.GetMembershipCertificates(certChains)) << "GetMembershipCertificates failed.";
        ASSERT_EQ(1U, certChains.size()) << "Bad membership cert chain.";
        ASSERT_EQ(3U, certChains[0].size()) << "Bad membership cert chain.";
    }
