...
class Participant : public SessionPortListener, public SessionListener {
  public:
    ...
    Participant() : bus("Participant"), name(genUniqueName(bus)),
...
void ObserverTest::SimpleScenario(Participant& provider, Participant& consumer)
{
    ...
    Observer obsA(consumer.bus, cintfA, 1, cintfB, 1);
    obsA.RegisterListener(listenerA);
    Observer obsB(consumer.bus, cintfB, 1, cintfA, 1);
    obsB.RegisterListener(listenerB);
    Observer obsAB(consumer.bus, cintfAB, 2, NULL, 0);
    ...
    Sleep(1000); /* this is slightly nasty, but there is little else we can do... */
    ...
    Observer obsB2(consumer.bus, cintfB, 1, cintfA, 1);
...
TEST_F(ObserverTest, Rejection)
{
    ...
    Observer obs(consumer.bus, cintfA, 1, NULL, 0);
...
TEST_F(ObserverTest, CreateDelete)
{
    ...
    spark = new Observer(consumer.bus, cintfA, 1, NULL, 0);
    delete spark;
    flame = new Observer(consumer.bus, cintfA, 1, NULL, 0);
...
TEST_F(ObserverTest, ListenTwice)
{
...

    {
        ...
        Observer obs2(consumer.bus, cintfA, 1, NULL, 0);
