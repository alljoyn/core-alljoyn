...
TEST_F(MultipleTrustAnchorsPropagationTest, Peer_With_Membership_Admin_claimed_with_CA1_as_CA) {
    ...
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SetCAFlagOnCert(busUsedAsCA1, identityCertChainMasterCA1[1])) << "Failed to set CA flag on CA1's cert";
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SetCAFlagOnCert(busUsedAsCA2, identityCertChainMasterCA2[1])) << "Failed to set CA flag on CA2's cert";
 ...
 * 1.
 * - Peer1 and Peer2 are claimed by Manager,
 * - Peer1's and identity certificates is signed by CA1,
 * - Peer1's and identity certificates is signed by CA2,
 * - Manager is an admin (member of the ASG),
 * - CA2 is the ASGA,
 * - Manager's identity certificate and ASG certificate are signed by CA2.
 * 2.
 * - Manager is claimed with CA1 as certificate authority.
 * 3.
 * - Peer1 and Peer2 have method "ping",
 * - On Peer1, method "ping" can be called only by peers whose identity is verified by CA2
 *   (a FROM_CERTIFICATE_AUTHORITY clause is added to Peer1's policy).
 ...
 * 4.
 * - Peer1 tries to call method "ping" of Peer2.
 * - Peer2 tries to call method "ping" of Peer1.
 *
 * Verification:
...
TEST_F(MultipleTrustAnchorsPropagationTest, Peer_From_Certificate_Authority) {
    ...
    EXPECT_EQ(ER_OK, peer1Bus.CreateInterfacesFromXml(s_interfaceXml.c_str()));
    TestBusObject peer1BusObject(peer1Bus, "/test", s_interfaceName.c_str());
    ...
    peer2Bus.GetPermissionConfigurator().SetApplicationState(PermissionConfigurator::CLAIMABLE);
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SignManifest(busUsedAsCA2, identityCertChainPeer2[0], manifests[0]));
    EXPECT_EQ(ER_OK, sapWithPeer2.Claim(ca2Key,
                                        managerGuid,
                                        ca2Key,
                                        identityCertChainPeer2, certChainSize,
                                        manifests, ArraySize(manifests)));
    ...
    /* Add a FROM_CERTIFICATE_AUTHORITY clause with CA2 key
     * to Peer1's policy.
     */
    ...
    EXPECT_EQ(ER_OK, peer1ToPeer2Proxy.ParseXml(s_interfaceXml.c_str()));
    EXPECT_TRUE(peer1ToPeer2Proxy.ImplementsInterface(s_interfaceName.c_str()));
    EXPECT_EQ(ER_OK, peer1ToPeer2Proxy.SecureConnection(true));
    ...
    EXPECT_EQ(ER_OK, peer2Bus.JoinSession(peer1Bus.GetUniqueName().c_str(), peer1SessionPort, nullptr, peer2ToPeer1SessionId, opts));

    /* Create the ProxyBusObject and call Peer1's "ping" method. */
    ProxyBusObject peer2ToPeer1Proxy(peer2Bus, peer1Bus.GetUniqueName().c_str(), "/test", peer2ToPeer1SessionId, true);
    EXPECT_EQ(ER_OK, peer2ToPeer1Proxy.ParseXml(s_interfaceXml.c_str()));
    EXPECT_TRUE(peer2ToPeer1Proxy.ImplementsInterface(s_interfaceName.c_str()));
    EXPECT_EQ(ER_OK, peer2ToPeer1Proxy.SecureConnection(true));

    Message peer2ToPeer1ReplyMsg(peer2Bus);
    EXPECT_EQ(ER_OK, peer2ToPeer1Proxy.MethodCall(s_interfaceName.c_str(), "ping", nullptr, static_cast<size_t>(0), peer2ToPeer1ReplyMsg));
}
