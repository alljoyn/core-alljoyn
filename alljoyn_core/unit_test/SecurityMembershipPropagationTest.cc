...
class MultipleTrustAnchorsPropagationTest : public testing::Test {
  public:
    MultipleTrustAnchorsPropagationTest() :
        ...
        busUsedAsLivingRoom("busUsedAsLivingRoom"),
    ...
    void registerKeystoreListeners() {
        ...
        EXPECT_EQ(ER_OK, busUsedAsCA1.RegisterKeyStoreListener(ca1KeyStoreListener));
        EXPECT_EQ(ER_OK, busUsedAsCA2.RegisterKeyStoreListener(ca2KeyStoreListener));
    ...
    void enableSecurity() {
        ...
        EXPECT_EQ(ER_OK, busUsedAsCA1.EnablePeerSecurity("ALLJOYN_ECDHE_NULL ALLJOYN_ECDHE_ECDSA", &ca1AuthListener));
        EXPECT_EQ(ER_OK, busUsedAsCA2.EnablePeerSecurity("ALLJOYN_ECDHE_NULL ALLJOYN_ECDHE_ECDSA", &ca2AuthListener));
    ...
    void startSessions() {
        ...
        EXPECT_EQ(ER_OK, managerBus.JoinSession(managerBus.GetUniqueName().c_str(), managerSessionPort, nullptr, managerToManagerSessionId, opts));
        EXPECT_EQ(ER_OK, managerBus.JoinSession(peer1Bus.GetUniqueName().c_str(), peer1SessionPort, nullptr, managerToPeer1SessionId, opts));
        EXPECT_EQ(ER_OK, managerBus.JoinSession(peer2Bus.GetUniqueName().c_str(), peer2SessionPort, nullptr, managerToPeer2SessionId, opts));
    }
    ...
    void getPublicKeys() {
        ...
        EXPECT_EQ(ER_OK, pcCA2.GetSigningPublicKey(ca2Key));
    ...
    void stopBusAttachments() {
        ...
        EXPECT_EQ(ER_OK, busUsedAsLivingRoom.Stop());
        EXPECT_EQ(ER_OK, busUsedAsLivingRoom.Join());
    }
...
/*
 * Purpose:
 * - Verify if an ASGA is trusted by a peer whose identity certificate is signed by a trust anchor
 *   different from the one which issued the ASGA's certificates,
 ...
 * Setup:
 * 1.
 * - Peer1 and Peer2 are claimed by ASGA,
 * - Peer1's and Peer2's identity certificates are signed by CA1,
 * - ASGA's identity certificate and ASG certificate are signed by CA2,
 * 2.
 * - ASGA is claimed with CA1 as certificate authority.
 * 3.
 * - Peer2 has method "ping",
 * - Method "ping" can be called only by members of group livingRoom, per Peer2's permission policy.
 * 4.
 * - Peer1 has a livingRoom membership certificate installed and signed by ASGA.
 * 5.
 ...
 */
TEST_F(MultipleTrustAnchorsPropagationTest, ASGA_claimed_with_CA1_as_CA) {
    ...
    EXPECT_EQ(ER_OK, peer2Bus.CreateInterfacesFromXml(s_interfaceXml.c_str()));
    ...
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SetCAFlagOnCert(busUsedAsCA1, identityCertChainMasterCA1[1])) << "Failed to set CA1 flag on CA's cert";
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SetCAFlagOnCert(busUsedAsCA2, identityCertChainMasterCA2[1])) << "Failed to set CA2 flag on CA's cert";

    /* SGA identity certificate to be signed by CA2. */
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::CreateIdentityCert(busUsedAsCA2,
                                                                  "0",
                                                                  ...
                                                                  "ManagerAlias",
                                                                  3600,
                                                                  identityCertChainMasterCA2[0])) << "Failed to create identity certificate.";
    ...
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SignManifest(busUsedAsCA2, identityCertChainMasterCA2[0], manifests[0]));
    /*
     * ASGA's identity certificate is signed by CA2.
     * Add CA1's key as ASGA's authority so that it recognizes
     * peer certificates which are issued by CA1 (see also next test).
     */
    EXPECT_EQ(ER_OK, sapWithManager.Claim(ca1Key,
                                          managerGuid,
                                          managerKey,
    ...
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SignManifest(busUsedAsCA1, identityCertChainPeer1[0], manifests[0]));
    EXPECT_EQ(ER_OK, sapWithPeer1.Claim(ca1Key,
                                        managerGuid,
                                        managerKey,
    ...
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SignManifest(busUsedAsCA1, identityCertChainPeer2[0], manifests[0]));
    EXPECT_EQ(ER_OK, sapWithPeer2.Claim(ca1Key,
                                        managerGuid,
                                        managerKey,
    ...
    /* ASGA ASG membership certificate to be signed by CA2. */
    ...
    /* Peer1 Living Room SG certificate to be signed by ASGA. */
    ...
    SessionOpts opts;
    EXPECT_EQ(ER_OK, peer1Bus.JoinSession(peer2Bus.GetUniqueName().c_str(), peer2SessionPort, nullptr, peer1ToPeer2SessionId, opts));
    ...
    EXPECT_EQ(ER_OK, proxy.ParseXml(s_interfaceXml.c_str()));
    EXPECT_TRUE(proxy.ImplementsInterface(s_interfaceName.c_str()));
 ...
 * - Verify if a group membership certificate is obtained and trusted by a peer whose identity
 *   certificate is signed by a trust anchor different from the one which issued the membership certificate,
 * - Verify if adding a FROM_CERTIFICATE_AUTHORITY clause to an ASGA's security policy makes the ASGA
 *   trust peers whose identity certificates have been issued by CAs previously unknown to the ASGA
 ...
 * - ASGA's identity certificate and ASG certificate are signed by CA2,
 * 2.
 * - ASGA is claimed with CA2 as certificate authority,
 * - A FROM_CERTIFICATE_AUTHORITY clause with CA1 is added to ASGA's policy.
 * 3.
 * - Peer2 has method "ping",
 * - Method "ping" can be called only by members of group livingRoom, per Peer2's permission policy.
 * 4.
 ...
 * - The call should succeed.
 */
TEST_F(MultipleTrustAnchorsPropagationTest, CA1_added_to_ASGA_in_policy) {
    SecurityApplicationProxy sapWithManager(managerBus, managerBus.GetUniqueName().c_str(), managerToManagerSessionId);
    SecurityApplicationProxy sapWithPeer1(managerBus, peer1Bus.GetUniqueName().c_str(), managerToPeer1SessionId);
    SecurityApplicationProxy sapWithPeer2(managerBus, peer2Bus.GetUniqueName().c_str(), managerToPeer2SessionId);
    ...
    /*
     * ASGA's identity certificate is signed by CA2.
     * ASGA's certificate authority is set to CA2.
     */
    EXPECT_EQ(ER_OK, sapWithManager.Claim(ca2Key,
                                          managerGuid,
                                          managerKey,
                                          identityCertChainMasterCA2, certChainSize,
                                          manifests, ArraySize(manifests)));
     ...
     * to the ASGA's policy so that peers with identity certs
     * signed by CA1 are authorized by the ASGA.
     */
    ...
    }
    {
        ...
        EXPECT_EQ(ER_OK, pcManager.UpdatePolicy(managerPolicy));
...
TEST(MemberShipPropagationTest, DISABLED_manager_updates_own_policy_via_remote_call) {
    ...
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SetCAFlagOnCert(busUsedAsCA, identityCertChainMasterCA[1])) << "Failed to set CA flag on CA's cert";
    ...
    managerBus.GetPermissionConfigurator().SetApplicationState(PermissionConfigurator::CLAIMABLE);
    EXPECT_EQ(ER_OK, PermissionMgmtTestHelper::SignManifest(busUsedAsCA, identityCertChainMasterCA[0], manifests[0]));
