...
QStatus PermissionMgmtTestHelper::InstallMembershipChain(const String& serial, BusAttachment& bus, const qcc::String& remoteObjName, BusAttachment& signingBus, BusAttachment& subjectBus, const qcc::GUID128& subjectGUID, const ECCPublicKey* subjectPubKey, const qcc::GUID128& guild, PermissionPolicy** authDataArray)
{
    PermissionMgmtProxy pmProxy(bus, remoteObjName.c_str());
    ...
    status = pmProxy.InstallMembershipAuthData(serial.c_str(), subjectGUID, *authDataArray[0]);
    if (ER_OK != status) {
        return status;
    }

    /* installing the auth data for second cert */
    status = pmProxy.InstallMembershipAuthData(serial.c_str(), localGUID, *authDataArray[1]);
    if (ER_OK != status) {
        return status;
    }
