                        ...
                        const char* annotation)
{
    ...
    EXPECT_EQ(ER_OK, status) << "  Actual Status: " << QCC_StatusText(status);
    QCC_DbgPrintf(("AddProperty(%s, emits=%s)", name, annotation));
}
                                                  ...
                                                  const InterfaceParameters& ip)
{
    ...
    if (NULL == intf) {
        ...
        QCC_DbgPrintf(("SetupInterface(%s)", ip.name.c_str()));
...
class PropChangedTestBusObject :
  ...
  public:
    ...
    PropChangedTestBusObject(BusAttachment& bus,
        ...
        intfParams(ip)
    {
        QCC_DbgPrintf(("PropChangedTestBusObject::constructor for %s", path));
    ...
    virtual QStatus Get(const char* ifcName, const char* propName, MsgArg& val)
    {
        ...
        val.v_int32 = num;
        QCC_DbgPrintf(("PropChangedTestBusObject::Get(%s, %s) -> %d", ifcName, propName, num));
        return ER_OK;
    }
                    ...
                    SessionId id = 0)
    {
        if (tp.newEmit) {
            ...
            QCC_DbgPrintf(("Emit for %s:%s in session %ud", ip.name.c_str(), s.str().c_str(), id));
#endif
            // signal
            ...
            for (int i = tp.rangePropEmit.first; i <= tp.rangePropEmit.last; i++) {
                name[1] = '0' + i;
                QCC_DbgPrintf(("Old emit for %s:%s on session %ud", ip.name.c_str(), name, id));
                MsgArg val("i", i);
...
class SampleStore {
  public:
                   ...
                   const MsgArg& invalidated)
    {
        ...
        copy = changed;
        changedSamples[ifaceName].push_back(copy);
        copy = invalidated;
        invalidatedSamples[ifaceName].push_back(copy);
        signalSema.Post();
        mutex.Unlock();
    }
...
class PropChangedTestListener :
  ...
  public:
    ...
    {
#if !defined(NDEBUG)
        const char* who = (const char*)context;
#endif
        ASSERT_EQ(ALLJOYN_ARRAY, changed.typeId);
        ASSERT_EQ(ALLJOYN_ARRAY, invalidated.typeId);
        QCC_DbgPrintf(("PropChangedTestListener::PropertiesChanged called (changed:%d,invalidated:%d) for %s",
                       changed.v_array.GetNumElements(), invalidated.v_array.GetNumElements(),
                       (who ? who : "?")));
                                 ...
                                 const TestParameters& tp)
{
    ...
    case PCM_INTROSPECT:
        QCC_DbgPrintf(("AddProxyInterface PCM_INTROSPECT"));
        status = proxy.IntrospectRemoteObject();
        break;

    case PCM_PROGRAMMATIC:
        QCC_DbgPrintf(("AddProxyInterface PCM_PROGRAMMATIC"));
        for (size_t i = 0; i < tp.intfParams.size(); i++) {
    ...
    case PCM_XML:
        ...
        QCC_DbgPrintf(("AddProxyInterface PCM_XML:\n%s", xml.c_str()));
...
class PropChangedTestProxyBusObject :
  ...
  public:
    ...
    PropChangedTestProxyBusObject(BusAttachment& bus,
                                  String serviceName,
                                  SessionId id,
                                  const TestParameters& tp,
                                  const char* path = OBJECT_PATH) :
        ProxyBusObject(bus, serviceName.c_str(), path, id),
        status(ER_OK),
        bus(bus)
    {
        QCC_DbgPrintf(("PropChangedTestProxyBusObject::constructor for %s", path));
        ...
        for (size_t num = 0; num < tp.rangePropListen.size(); num++) {
            ...
            for (size_t intf = 0; intf < tp.intfParams.size(); intf++) {
                ...
                QCC_DbgPrintf(("Register listener %d for %s", num, name.c_str()));
                          ...
                          const char* who = NULL)
    {
        if (props.Size() > 0) {
            ...
            ostringstream s;
            copy(propertyNames.begin(), propertyNames.end(), ostream_iterator<const char*>(s, ","));
            QCC_DbgPrintf(("Listener for properties %s", s.str().c_str()));
#endif
            ...
            QCC_DbgPrintf(("Listener for all properties"));
            status = RegisterPropertiesChangedListener(ifaceName.c_str(),
                                                       NULL, 0, *listener, NULL);
        }
        EXPECT_EQ(ER_OK, status) << "  Actual Status: " << QCC_StatusText(status);
    }
                        ...
                        uint32_t expectTimeout = 0)
    {
        ...
        for (size_t i = 0; i < num; i++) {
            QCC_DbgPrintf(("WaitForSignals %dms on listener %d", timeout, i));
                         ...
                         const InterfaceParameters& ip)
    {
        ...
        QCC_DbgPrintf(("ValidateSignal expects %d samples with each %d changed, %d invalidated",
    ...
    {
        for (size_t i = 0; i < tp.intfParams.size(); i++) {
            QCC_DbgPrintf(("ValidateSignal for %s", tp.intfParams[i].name.c_str()));
...
class PropChangedTestTwoBusSetup :
    ...
    private BusListener,
    protected BusAttachment::JoinSessionAsyncCB {
  public:
    QStatus status;
    ...
    PropChangedTestTwoBusSetup() :
        status(ER_FAIL),
        clientBus("PropChangedTestClient", false),
    ...
    void SetUp()
    {
        ...
        ASSERT_EQ(ER_OK, status) << "  Actual Status: " << QCC_StatusText(status);
        clientBus.RegisterBusListener(*this);
        status = clientBus.FindAdvertisedName(serviceName.c_str());
        ASSERT_EQ(ER_OK, status) << "  Actual Status: " << QCC_StatusText(status);
    }
                          ...
                          const TestParameters& tpClient)
    {
        ...
        proxy = new PropChangedTestProxyBusObject(clientBus, serviceName, id[0], tpClient);
    ...
    {
        peerName = name;
        clientBus.JoinSessionAsync(name, SERVICE_PORT, NULL, SESSION_OPTS, this, NULL);
    }
                       ...
                       void* context)
    {
        ...
        id.push_back(sessionId);
        sessionSema.Post();
    }
};

class EmitPropChanged :
    public PropChangedTestTwoBusSetup,
    public testing::TestWithParam<InterfaceParameters> {
  public:
    EmitPropChanged() : PropChangedTestTwoBusSetup() { }
    virtual ~EmitPropChanged() { }
    virtual void SetUp() { PropChangedTestTwoBusSetup::SetUp(); }
    virtual void TearDown() { PropChangedTestTwoBusSetup::TearDown(); }
};

/**
 * Test property emission:
 * -# for single or multiple properties
 * -# for different types of annotations
 */
TEST_P(EmitPropChanged, Default)
{
    TestParameters tp(true, GetParam());
    TestPropChanged(tp);
}
 ...
 */
INSTANTIATE_TEST_CASE_P(PropChangedTest, EmitPropChanged,
                        testing::Values(
                            ...
                            InterfaceParameters(P1, "true", true),
                            /* Create a BusObject containing an interface with
                            ...
                            InterfaceParameters(P1to4, "true", true),
                            /* See above but with annotation "invalidates". */
                            InterfaceParameters(P1, "invalidates", true),
                            /* See above but with annotation "invalidates". */
                            InterfaceParameters(P1to4, "invalidates", true)));

class PropertiesChangedListener :
    public PropChangedTestTwoBusSetup,
    public testing::TestWithParam<TestParameters> {
  public:
    PropertiesChangedListener() : PropChangedTestTwoBusSetup() { }
...
/**
 * Test property changed listener.
 */
TEST_P(PropertiesChangedListener, Default)
{
 ...
 *     - EmitsChanged annotation value
 */
INSTANTIATE_TEST_CASE_P(PropChangedTest, PropertiesChangedListener,
                        testing::Values(
                            ...
                            TestParameters(false, P1, P1, PCM_INTROSPECT,
                                           InterfaceParameters(P1)),
                            TestParameters(false, P1, P1, PCM_PROGRAMMATIC,
                                           InterfaceParameters(P1)),
                            TestParameters(false, P1, P1, PCM_XML,
                                           InterfaceParameters(P1)),
                            /* Register a single listener for a property P
                             ...
                             * called with P1. */
                            TestParameters(true, P1, P1, PCM_INTROSPECT,
                                           InterfaceParameters(P1)),
                            TestParameters(true, P1, P1, PCM_PROGRAMMATIC,
                                           InterfaceParameters(P1)),
                            TestParameters(true, P1, P1, PCM_XML,
                                           InterfaceParameters(P1)),
                             ...
                             * is called with P. */
                            TestParameters(true, P1, P1, PCM_INTROSPECT,
                                           InterfaceParameters(P1, "invalidates")),
                            TestParameters(true, P1, P1, PCM_PROGRAMMATIC,
                                           InterfaceParameters(P1, "invalidates")),
                            TestParameters(true, P1, P1, PCM_XML,
                                           InterfaceParameters(P1, "invalidates")),
                            ...
                            TestParameters(true, P1to3, P1, PCM_INTROSPECT,
                                           InterfaceParameters(P1to3)),
                            TestParameters(true, P1to3, P1, PCM_PROGRAMMATIC,
                                           InterfaceParameters(P1to3)),
                            TestParameters(true, P1to3, P1, PCM_XML,
                                           InterfaceParameters(P1to3)),
                            /* Register a single listener for specific
                             * properties P1, P2 and P3 of interface I.
                             ...
                             * P3. */
                            TestParameters(true, P1to3, P1to3, PCM_INTROSPECT,
                                           InterfaceParameters(P1to3)),
                            TestParameters(true, P1to3, P1to3, PCM_PROGRAMMATIC,
                                           InterfaceParameters(P1to3)),
                            TestParameters(true, P1to3, P1to3, PCM_XML,
                                           InterfaceParameters(P1to3)),
                            ...
                            TestParameters(true, Pall, P1to3, PCM_INTROSPECT,
                                           InterfaceParameters(P1to3)),
                            TestParameters(true, Pall, P1to3, PCM_PROGRAMMATIC,
                                           InterfaceParameters(P1to3)),
                            TestParameters(true, Pall, P1to3, PCM_XML,
                                           InterfaceParameters(P1to3))
                            ));

class PropertiesChangedListener2 :
    public PropChangedTestTwoBusSetup,
    public testing::TestWithParam<ProxyCreationMethod> {
  public:
    PropertiesChangedListener2() : PropChangedTestTwoBusSetup() { }
    virtual ~PropertiesChangedListener2() { }
    virtual void SetUp() { PropChangedTestTwoBusSetup::SetUp(); }
    virtual void TearDown() { PropChangedTestTwoBusSetup::TearDown(); }
};
...
TEST_P(PropertiesChangedListener2, MultiInterface)
{
    TestParameters tp(true, P1, P1, GetParam());
...
TEST_P(PropertiesChangedListener2, TwoHalves)
{
    TestParameters tp(true, P1to2, P1to4, GetParam(), InterfaceParameters(P1to4));
 ...
 * get called with P2. */
TEST_P(PropertiesChangedListener2, OverlappingHalves)
{
    TestParameters tp(true, P2, GetParam());
    tp.AddInterfaceParameters(InterfaceParameters(P1to3));
 ...
 * via raw xml, programmatically).
 */
INSTANTIATE_TEST_CASE_P(PropChangedTest, PropertiesChangedListener2,
                        testing::Values(
                            PCM_INTROSPECT,
                            PCM_PROGRAMMATIC,
                            PCM_XML));

class PropertiesChangedTestExtra :
    public PropChangedTestTwoBusSetup,
    public testing::Test {
  public:
    PropertiesChangedTestExtra() : PropChangedTestTwoBusSetup() { }
    virtual ~PropertiesChangedTestExtra() { }
    virtual void SetUp() { PropChangedTestTwoBusSetup::SetUp(); }
    virtual void TearDown() { PropChangedTestTwoBusSetup::TearDown(); }
};
 ...
 * invoked.
 */
TEST_F(PropertiesChangedTestExtra, PartialProxy)
{
 ...
 *   - and vice versa
 */
TEST_F(PropertiesChangedTestExtra, DISABLED_MultiSession)
{
    ...
    clientBus.JoinSessionAsync(peerName.c_str(), SERVICE_PORT, NULL, SESSION_OPTS, this, NULL);
    WaitForSession(); // S2
    ...
    PropChangedTestProxyBusObject pb1(clientBus, serviceName, id[0], tp, OBJECT_PATH "/BobA");
    PropChangedTestProxyBusObject pb2(clientBus, serviceName, id[1], tp, OBJECT_PATH "/BobA");
    PropChangedTestProxyBusObject pb3(clientBus, serviceName, id[0], tp, OBJECT_PATH "/BobB");
    ...
    bobA.EmitSignal(tp, tp.intfParams[0], id[0]);
    qcc::Sleep(500);
    ...
    store.Clear();
    EXPECT_EQ((size_t)0, store.proxySamples.size());
    bobA.EmitSignal(tp, tp.intfParams[0], id[1]);
    qcc::Sleep(500);
    // status = store.TimedWait(TIMEOUT); // wait for property changed signal
    // EXPECT_EQ(ER_OK, status) << "  Actual Status: " << QCC_StatusText(status);
    // status = store.TimedWait(TIMEOUT); // wait for property changed signal
    ...
    store.Clear();
    EXPECT_EQ((size_t)0, store.proxySamples.size());
    bobB.EmitSignal(tp, tp.intfParams[0], id[0]);
    qcc::Sleep(500);
    // status = store.TimedWait(TIMEOUT); // wait for property changed signal
    // EXPECT_EQ(ER_OK, status) << "  Actual Status: " << QCC_StatusText(status);
    EXPECT_EQ((size_t)1, store.proxySamples.size());
...
class PropertiesChangedTestNegative :
    public PropChangedTestTwoBusSetup,
    public testing::Test {
  public:
    PropertiesChangedTestNegative() : PropChangedTestTwoBusSetup() { }
    virtual ~PropertiesChangedTestNegative() { }
    virtual void SetUp() { PropChangedTestTwoBusSetup::SetUp(); }
    virtual void TearDown() { PropChangedTestTwoBusSetup::TearDown(); }
};
 ...
 */
TEST_F(PropertiesChangedTestNegative, EmitPropChanged)
{
 ...
 */
TEST_F(PropertiesChangedTestNegative, RegisterPropertiesChangedListener)
{
    TestParameters tp(true, InterfaceParameters(P1));
    const char* okProps[] = { "P1" };
    const char* nokProps[] = { "P2" };
    ...
    WaitForSession();
    PropChangedTestBusObject obj(serviceBus, tp.intfParams);
    PropChangedTestProxyBusObject proxy(clientBus, serviceName, id[0], tp);
 ...
 */
TEST_F(PropertiesChangedTestNegative, UnregisterPropertiesChangedListener)
{
    TestParameters tp(true, InterfaceParameters(P1));
 ...
 */
TEST_F(PropertiesChangedTestNegative, DISABLED_UnregisterInvalidPropertiesChangedListener)
{
    TestParameters tp(true, InterfaceParameters(P1));
...
class PropertiesChangedTestListenerNotCalled :
    public PropChangedTestTwoBusSetup,
    public testing::Test {
  public:
    PropertiesChangedTestListenerNotCalled() : PropChangedTestTwoBusSetup() { }
    virtual ~PropertiesChangedTestListenerNotCalled() { }
    virtual void SetUp() { PropChangedTestTwoBusSetup::SetUp(); }
    virtual void TearDown() { PropChangedTestTwoBusSetup::TearDown(); }
};
 ...
 */
TEST_F(PropertiesChangedTestListenerNotCalled, AfterUnregister)
{
    TestParameters tp(true, InterfaceParameters(P1));
 ...
 */
TEST_F(PropertiesChangedTestListenerNotCalled, PropertyNotListeningNew)
{
 ...
 * NOT get called.
 */
TEST_F(PropertiesChangedTestListenerNotCalled, InterfaceNotListening)
{
    TestParameters tp(true, P1);
    tp.AddInterfaceParameters(InterfaceParameters(P1, "true", false, INTERFACE_NAME "1"));
    tp.AddInterfaceParameters(InterfaceParameters(P1, "true", false, INTERFACE_NAME "2"));
 ...
 * does NOT get called.
 */
TEST_F(PropertiesChangedTestListenerNotCalled, AnnotationFalse)
{
 ...
 */
TEST_F(PropertiesChangedTestListenerNotCalled, PropertyNotListeningOld)
{
 ...
 * listener does NOT get called.
 */
TEST_F(PropertiesChangedTestListenerNotCalled, PropertyNotListeningNewMulti)
{
 ...
 */
TEST_F(PropertiesChangedTestListenerNotCalled, PropertyEmitOneProp)
{
    TestParameters tp(true, P1);
 ...
 * NOT get called.
 */
TEST_F(PropertiesChangedTestListenerNotCalled, PropertyEmitOneIntf)
{
    TestParameters tp(true, P1);
    tp.AddInterfaceParameters(InterfaceParameters(P1, "true", false, INTERFACE_NAME "1"));
    tp.AddInterfaceParameters(InterfaceParameters(P1, "true", false, INTERFACE_NAME "2"));
 ...
 */
TEST_F(PropertiesChangedTestListenerNotCalled, PartialProxy)
{
    // one interface for client
    TestParameters tpClient(true, P1, P1, PCM_INTROSPECT);
    tpClient.AddInterfaceParameters(InterfaceParameters(P1, "true", false, INTERFACE_NAME "1"));
