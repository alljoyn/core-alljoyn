...
TEST_P(DaemonRouterTest, PushMessage)
{
    ...
    /*
     * policyError is not always the accurate expectation, since the receiverDenied could be updated in iterations of
     * the loop for each endpoint, also in the following situations, it should not be expected when senderDenied or receiverDenied is true:
     *
     ...
     *
     * 2. if msgIsBroadcast, for SendSignalsCast test cases, the destination would be all EPs including local EP where policyDB rules do not
