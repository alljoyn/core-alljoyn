...
#define SLEEP_TIME (500 * GlobalTimerMultiplier)
...
TEST_F(RemoteEndpointTest, OrderlyReleaseWhenTxQueueIsEmpty)
{
    ...
    EXPECT_EQ(ER_OK, rep->Join(40 * 1000 * GlobalTimerMultiplier));
...
TEST_F(RemoteEndpointTest, TxTimeout)
{
    ...
    EXPECT_EQ(ER_OK, trep->Join(40 * 1000 * GlobalTimerMultiplier));
...
TEST_F(RemoteEndpointTest, TxQueueIsFull)
{
    ...
    qcc::Sleep(SLEEP_TIME); /* Wait for PushMessages thread to block */
    EXPECT_EQ(ER_OK, rep->Stop());
    ts.sinkEvent.SetEvent();
    ts.sourceEvent.SetEvent();
    EXPECT_EQ(ER_OK, rep->Join(40 * 1000 * GlobalTimerMultiplier));
