...
static bool SessionJoinLeaveTest(BusAttachment& busHost, BusAttachment& busJoiner, bool joinerLeaves, bool multipoint) {
    ...
    if (joinerLeaves) {
        ...
        if (&busHost == &busJoiner && multipoint == true) {
            EXPECT_EQ(ER_FAIL, busHost.LeaveHostedSession(sessionId)); /* must first leave as joiner */
            return true;
        } else {
...
static void MultipointMultipeerTest(BusAttachment& busHost, BusAttachment& busJoiner, BusAttachment& busJoiner2, SessionAction sa) {
    ...
    case HOST_LEAVES:
        {
            if (&busHost == &busJoiner || &busHost == &busJoiner2) {
                EXPECT_EQ(ER_FAIL, busHost.LeaveHostedSession(sessionId));
...
/* Test is disabled because the usecase where you remove yourself from a multipoint session is currently not supported */
TEST_F(SessionTest, DISABLED_MultipointSelfJoinRemoveMember) {
