...
class RemoveSessionMemberBusAListener : public SessionPortListener, public SessionListener {
  public:
    ...
    virtual void SessionJoined(SessionPort sessionPort, SessionId id, const char* joiner) {
        ...
        sessionJoinedFlag = true;
        ++sessionJoinedCounter;
        EXPECT_EQ(ER_OK, bus->SetSessionListener(id, this));
    }
    virtual void SessionLost(SessionId sessionId, SessionLostReason reason) {
...
TEST_F(SessionTest, MultipointExtended_AA_B_2ndJoiner_A_removes_A_as_joiner) {
...
TEST(SessionSystemTest, DISABLED_MultipointExtended_AA_B_2ndJoiner_A_removes_A_as_joiner_forked) {
    printf("A - A , B as 2nd joiner (self-join), A removes A as joiner, A and B run in their own process\n");
    pid_t child;
    int procstatus;
    SessionOpts opts(SessionOpts::TRAFFIC_MESSAGES, true, SessionOpts::PROXIMITY_ANY, TRANSPORT_ANY);
    ...
    if ((child = fork()) == 0) {
        ...
        //Wait upto 3 seconds all callbacks and listeners to be called.
        for (int i = 0; i < 300; ++i) {
            if (sessionListenerJoiner2.sessionMemberAddedCalled == 2) {
                break;
            }
            qcc::Sleep(10);
        }

        ASSERT_EQ(2, sessionListenerJoiner2.sessionMemberAddedCalled);
        ...
        ASSERT_EQ(0, sessionListenerJoiner2.sessionLostCalled);
