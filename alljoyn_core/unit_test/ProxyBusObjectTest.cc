...
#define SLEEP_TIME_5 (5 * GlobalTimerMultiplier)
#define SLEEP_TIME_10 (10 * GlobalTimerMultiplier)
#define TIMEOUT_100 (100 * GlobalTimerMultiplier)
#define TIMEOUT_1000 (1000 * GlobalTimerMultiplier)
  ...
  public:
    ...
    ThreadReturn STDCALL Run(void* arg)
    {
        ...
        while (wait && (status == ER_OK)) {
            ...
            if (wait) {
                status = testCond.TimedWait(testLock, TIMEOUT_1000);
            }
        }
        testLock.Unlock();
        if (l && status == ER_OK) {
...
TEST_F(ProxyBusObjectTest, UnregisterPropertiesChangedListenerRaceTest1) {
    ...
    while (!removeListener1.didRun && !removeListener2.didRun) {
        status = cond.TimedWait(lock, TIMEOUT_1000);  // Should not timeout
        if (status != ER_OK) {
            break;
        }
    }
    ...
    if (!(removeListener1.didRun && removeListener2.didRun)) {
        status = cond.TimedWait(lock, TIMEOUT_100);  // Should timeout
