...
class TestBusObject : public BusObject {
  public:
    TestBusObject(BusAttachment& bus, const char* path, const char* interfaceName) :
        BusObject(path)
    {
...
class IntrospectionXmlTest : public::testing::Test {
  protected:
    ...
    void CreateFromXmlAndIntrospect(const char* input, const char* output = nullptr)
    {
        ...
        ASSERT_TRUE((m_testObj = new TestBusObject(*m_msgBusServer, SERVICE_PATH, INTERFACE_NAME)) != nullptr);
...
TEST_F(IntrospectionXmlTest, IntrospectionXmlCreateInterfacesFromXmlWithSignalEmissionBehaviors) {
    ...
    ASSERT_TRUE(member != 0);
    EXPECT_EQ(false, member->isSessionlessSignal);
    EXPECT_EQ(false, member->isGlobalBroadcastSignal);
    EXPECT_EQ(false, member->isSessioncastSignal);
    EXPECT_EQ(false, member->isUnicastSignal);

    member = testIntf->GetSignal("globalBroadcastSignal");
    ASSERT_TRUE(member != 0);
    EXPECT_EQ(false, member->isSessionlessSignal);
    EXPECT_EQ(true, member->isGlobalBroadcastSignal);
    ...
    ASSERT_TRUE(member != 0);
    EXPECT_EQ(true, member->isSessioncastSignal);
    EXPECT_EQ(false, member->isSessionlessSignal);

    member = testIntf->GetSignal("sessionlessSignal");
    ASSERT_TRUE(member != 0);
    EXPECT_EQ(true, member->isSessionlessSignal);

    member = testIntf->GetSignal("unicastSignal");
    ASSERT_TRUE(member != 0);
    EXPECT_EQ(false, member->isSessionlessSignal);
    EXPECT_EQ(true, member->isUnicastSignal);
}
