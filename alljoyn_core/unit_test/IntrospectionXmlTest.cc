...
TEST_F(IntrospectionXmlTest, IntrospectionXmlCreateInterfacesFromXmlWithDescriptions) {
    ...
    EXPECT_TRUE(testIntf->GetMemberDescriptionForLanguage("LightOn", description, "Nl"));
    EXPECT_STREQ("Licht is aangestoken", description.c_str());
    EXPECT_TRUE(testIntf->GetPropertyDescriptionForLanguage("SupplySource", description, "Nl"));
...
TEST_F(IntrospectionXmlTest, IntrospectionXmlCreateInterfacesFromXmlLegacy) {
    ...
    ASSERT_TRUE(member != 0);
    EXPECT_FALSE(member->isSessionlessSignal);
    EXPECT_TRUE(testIntf->GetMemberDescriptionForLanguage("legacyNonSessionlessSignal", description, "en"));
    EXPECT_STREQ("legacy non-sessionless signal", description.c_str());

    member = testIntf->GetSignal("legacySessionlessSignal");
    ASSERT_TRUE(member != 0);
    EXPECT_TRUE(member->isSessionlessSignal);
    EXPECT_TRUE(testIntf->GetMemberDescriptionForLanguage("legacySessionlessSignal", description, "en"));
