...
QStatus JBusObject::Get(const char* ifcName, const char* propName, MsgArg& val, qcc::String& errorName, qcc::String& errorMessage)
{
    ...
    if (exception) {
        ...
        if (env->IsInstanceOf(exception, CLS_ErrorReplyBusException)) {
            ...
            QCC_DbgPrintf(("JBusObject::Get errorName: %s errorMessage: %s", errorName.c_str(), errorMessage.c_str()));
            mapLock.Unlock();
...
QStatus JBusObject::Set(const char* ifcName, const char* propName, MsgArg& val, qcc::String& errorName, qcc::String& errorMessage)
{
    ...
    if (exception) {
        ...
        if (env->IsInstanceOf(exception, CLS_ErrorReplyBusException)) {
            ...
            QCC_DbgPrintf(("JBusObject::Set errorName: %s errorMessage: %s", errorName.c_str(), errorMessage.c_str()));
            mapLock.Unlock();
            return ER_BUS_REPLY_IS_ERROR_MESSAGE;
...
JNIEXPORT jobject JNICALL Java_org_alljoyn_bus_BusAttachment_registerApplicationStateListener(JNIEnv* env, jobject thiz, jobject jappstatelistener)
{
    ...
    if (status == ER_OK) {
        busPtr->baAppStateListenLock.Lock();
        busPtr->jApplicationStateListeners.push_back(jASListener);
        busPtr->baAppStateListenLock.Unlock();
    } else {
        delete jASListener;
    }
 ...
 */
JNIEXPORT jobject JNICALL Java_org_alljoyn_bus_BusAttachment_getGuid(JNIEnv* env, jobject thiz)
{
    QCC_DbgTrace(("%s", __FUNCTION__));
    ...
    CredentialAccessor ca(*busPtr);

    QStatus status = ca.GetGuid(guid);
    ...
    jbyteArray jbyteGuid = ToJByteArray(reinterpret_cast<const uint8_t*>(strGuid.data()), strGuid.size());

    jmethodID mid = env->GetStaticMethodID(CLS_JAVA_UTIL_UUID, "nameUUIDFromBytes", "([B)Ljava/util/UUID;");
    if (!mid) {
        return NULL;
    }

    return env->CallStaticObjectMethod(CLS_JAVA_UTIL_UUID, mid, jbyteGuid);
}
 ...
 */
JNIEXPORT jobject JNICALL Java_org_alljoyn_bus_BusAttachment_getDSAPrivateKey(JNIEnv* env, jobject thiz)
{
    QCC_DbgTrace(("%s", __FUNCTION__));
    ...
    jbyteArray jeccD = ToJByteArray(privateKey.GetD(), privateKey.GetDSize());
    ...
    return env->NewObject(CLS_ECCPrivateKey, MID_ECCPrivateKey_cnstrctr, jeccD);
}
                                                                         ...
                                                                         jint flags)
{
    ...
    if (member->GetAnnotation(org::freedesktop::DBus::AnnotateNoReply, val) && val == "true") {
        ...
        if (ER_OK == status) {
            ...
            if (errorName) {
                if (!strcmp(org::alljoyn::Bus::ErrorName, errorName)) {
                                                                          ...
                                                                          jstring jinterfaceName, jstring jpropertyName)
{
    ...
    if (ER_OK == status) {
        ...
        QCC_LogError(status, ("ProxyBusObjexct_getProperty(): Exception"));
        if (!errorName.empty()) {
            QCC_DbgPrintf(("ProxyBusObject_getProperty(): Exception ErrorName: %s\nException ErrorMessage: %s", (errorName.c_str(), errorMessage.c_str())));
            /* errorName is returned as org.alljoyn.Bus.ErStatus
             * if the exception thrown has a null or empty "" string.
             * This occurs when the exception is a BusException or any of
             * the children of BusException besides ErrorReplyBusException.
             * This also occurs if the exception thrown is an ErrorReplyBusException
             * with an empty errorName.
             */
            if (!strcmp(org::alljoyn::Bus::ErrorName, errorName.c_str())) {
                env->ThrowNew(CLS_BusException, errorMessage.c_str());
            } else {
...
JPingListener::JPingListener(jobject jlistener) : jpinglistener(NULL)
{
    ...
    mid_DestinationLost = env->GetMethodID(clazz, "destinationLost", "(Ljava/lang/String;Ljava/lang/String;)V");
    if (!mid_DestinationLost) {
        QCC_DbgPrintf(("JPingListener::JPingListener(): Can't find DestinationLost() in PingListener"));
    }

    mid_DestinationFound = env->GetMethodID(clazz, "destinationFound", "(Ljava/lang/String;Ljava/lang/String;)V");
