...
JNIEXPORT void JNICALL Java_org_alljoyn_bus_SecurityApplicationProxy_claim(JNIEnv* jenv, jobject thiz, jobject jcertAuth, jobject jadminGroupId, jobject jadminGroup, jobjectArray jcertArray, jlong jcertChainCount, jobjectArray jmanifestobjs, jlong jmaniCount)
{
    ...
    QStatus status = eccPublicKey.Import(eccX, sizeof(eccX), eccY, sizeof(eccY));
    ...
    certificateAuthority.SetKeyId(keyID, sizeof(keyID));
    ...
    status = eccPublicKeyAdminGroup.Import(adminGroupEccX, sizeof(adminGroupEccX), adminGroupEccY, sizeof(adminGroupEccY));
    ...
    certificateAuthority.SetKeyId(adminGroupKeyID, sizeof(adminGroupKeyID));
...
exit:
    /*
     * I think the Java garbage collector should take care of the certArray
     * because of the function destroy in java CertificateX509
     */
    certChain = NULL;
...
JNIEXPORT jstring JNICALL Java_org_alljoyn_bus_SecurityApplicationProxy_signManifest(JNIEnv* jenv, jclass, jobject identityCertificate, jobject jeccPrivateKey, jstring junsignedManifestXml)
{
    ...
    QStatus status = eccPrivateKey.Import(eccD, sizeof(eccD));
    ...
    return jenv->NewStringUTF(retStr[0]);
}
