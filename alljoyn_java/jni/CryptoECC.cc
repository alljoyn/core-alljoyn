...
JNIEXPORT jobject JNICALL Java_org_alljoyn_bus_common_CryptoECC_DSASignDigest(JNIEnv* jenv, jobject thiz, jbyteArray jdigest, jint jdigestlen)
{
    ...
    uint8_t * eccsigr = new uint8_t[eccsignature->GetSize() / 2];
    uint8_t * eccsigs = new uint8_t[eccsignature->GetSize() / 2];

    std::copy(eccbuffer, eccbuffer + eccsignature->GetSize() / 2, eccsigr);
    std::copy(eccbuffer + eccsignature->GetSize() / 2, eccbuffer + eccbufferlen, eccsigs);
