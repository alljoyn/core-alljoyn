...
public class InterfaceDefTest extends TestCase {
    ...
    public void testBusObjectInfo() throws Exception {
        ...
        interfaces.add( new InterfaceDef("org.alljoyn.bus.iface1") ); // presumably interface has methods/signals/properties
        interfaces.add( new InterfaceDef("org.alljoyn.bus.iface2") );
        interfaces.add( new InterfaceDef("org.alljoyn.bus.iface3") );

        // Define Bus Object info
        BusObjectInfo busInfo = new BusObjectInfo(path, interfaces);

        assertEquals(3, busInfo.getInterfaces().size());
    }
