    ...
    extends TestCase
{
        ...
        extends ProxyBusObject
    {
        ...
        private final List<PropertiesChangedListener> mListeners;

        public PropChangedTestProxyBusObject(ClientBusAttachment bus, TestParameters tp, Class<?>[] busInterfaces)
            throws BusException
        {
            super(bus, bus.getPeerName(), OBJECT_PATH, bus.getSessionId(), busInterfaces);
            mStore = new SampleStore();
            mListeners = new ArrayList<PropertiesChangedListener>();
            ...
            for (Range listenRange : tp.rangePropListen) {
                for (InterfaceParameters ip : tp.intfParams) {
                    ...
                    if (listenRange.size() > 0) {
                        Set<String> propSet = getPropertyNameList(listenRange);
                        String[] prop = propSet.toArray(new String[propSet.size()]);
                        assertEquals(Status.OK, registerPropertiesChangedListener(ip.name, prop, l));
                    }
                    else {
                        assertEquals(Status.OK, registerPropertiesChangedListener(ip.name, new String[] {}, l));
                    }
                    mListeners.add(l);
                }
            }
        ...
        throws BusException
    {
        ...
        return new PropChangedTestProxyBusObject(bus, tp, intf);
    }
    ...
    private class ServiceBusObject
        ...
        throws BusException
    {
        ...
        mObj = new ServiceBusObject(serviceBus);
        mProxy = createProxy(clientBus, tp);
        PropertiesChangedListener invalidListener = new MyPropertiesChangedListener(new SampleStore());
