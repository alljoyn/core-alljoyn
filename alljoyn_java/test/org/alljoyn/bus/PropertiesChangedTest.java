    ...
    extends TestCase
{
        ...
        throws Exception
    {
        ...
        assertEquals("Prop1", getChangedPropName());
        assertEquals("Hello", getChangedPropValue());
        assertEquals("Prop2", getInvalidatedPropName());
    }
        ...
        public void propertiesChanged(ProxyBusObject pObj, String ifaceName, Map<String, Variant> changed, String[] invalidated)
        {
            try {
                for (Map.Entry<String, Variant> entry : changed.entrySet()) {
                    setChangedPropName(entry.getKey());
                    setChangedPropValue(entry.getValue().getObject(String.class));
                    System.out.println("Prop changed: " + getChangedPropName() + " = " + getChangedPropValue());
                }
                for (String propName : invalidated) {
                    setInvalidatedPropName(propName);
                    System.out.println("Prop invalidated: " + getInvalidatedPropName());
                }
    ...
    public synchronized String getChangedPropName() {
        return changedPropName;
    }

    public synchronized void setChangedPropName(String name) {
        changedPropName = name;
    }

    public synchronized String getChangedPropValue() {
        return changedPropValue;
    }

    public synchronized void setChangedPropValue(String value) {
        changedPropValue = value;
    }

    public synchronized String getInvalidatedPropName() {
        return invalidatedPropName;
    }

    public synchronized void setInvalidatedPropName(String name) {
        invalidatedPropName = name;
    }
