...
public class SignalEmitterTest extends TestCase {
    ...
    public class DynamicEmitter implements DynamicBusObject {
        ...
        /**
         * Signal handler for a signal received on consumer-side.
         * Method mapped to specific signal via DynamicBusObject.getSignalHandler().
         *
         * Note: It is matched (invoked) on in-session signaling when XXXXX,
         * or on sessionless signaling after registerSignalHandlers(DynamicBusObject).
         */
        public void receiveSignal(Object... args) throws BusException {
            receivedSignalInDynBusObj++;

            ctx = ownerBus.getMessageContext();
            String msg = (args.length == 1) ? (String)args[0] : Arrays.toString(args);
    ...
    private static BusObjectInfo buildBusObjectInfo(String path, String signalRule) {
        SignalDef signalDef = new SignalDef(DYN_SIGNAL_NAME, "s", DYN_IFACE_NAME, signalRule);
...
//        daemon.stop();
    ...
    /** Shows workaround for 16.10 registerSignalHandler() fail with error BUS_NO_SUCH_INTERFACE. */
    public void testDynamicSignalDef_registerHandler_1610Workaround() throws Exception {
        Status status;
        ...
        /*
         * 16.10 WORKAROUND: The 16.10 registerSignalHandler() fails with error "no such interface"
         * when registering for a dynamic interface. It does not create the dynamic interface as needed.
         * Consumer-side will have to explicitly create the interface until a permanent fix is available.
         */
        InterfaceDescription ifaceDesc = new InterfaceDescription(null);
        status = ifaceDesc.create(clientBus, busObjInfo.getInterface(DYN_IFACE_NAME));
        assertEquals(Status.OK, status);

        // Register the client signal handler (consumer-side) for a specific signal
        status = clientBus.registerSignalHandler(DYN_IFACE_NAME, DYN_SIGNAL_NAME, this,
        ...
        dynamicEmitter.emitSignal(DYN_IFACE_NAME, DYN_SIGNAL_NAME, "registerHandler_1610Workaround");
    ...
    public void testDynamicSignalDef_registerHandlers() throws Exception {
        ...
        DynamicBusObject busObjHandler = new DynamicEmitter(clientBus, buildBusObjectInfo("","")); // empty path and rule
    ...
    public void testDynamicSignalDef_registerHandlers_withSourcePath() throws Exception {
        ...
        DynamicBusObject busObjHandler = new DynamicEmitter(
            clientBus, buildBusObjectInfo(DYN_PATH_NAME,"")); // non-empty path, empty rule
        status = clientBus.registerSignalHandlers(busObjHandler);
        assertEquals(Status.OK, status);
    ...
    public void testDynamicSignalDef_registerHandlers_withBogusSourcePath() throws Exception {
        ...
        DynamicBusObject busObjHandler = new DynamicEmitter(
            clientBus, buildBusObjectInfo("/bogusEmitter","")); // bogus path, empty rule
        status = clientBus.registerSignalHandlers(busObjHandler);
        assertEquals(Status.OK, status);
    ...
    public void testDynamicSignalDef_registerHandlers_withRule() throws Exception {
        ...
        String rule = "path='" + DYN_PATH_NAME + "'";
        DynamicBusObject busObjHandler = new DynamicEmitter(
            clientBus, buildBusObjectInfo("",rule)); // empty path, non-empty rule
        status = clientBus.registerSignalHandlers(busObjHandler);
        assertEquals(Status.OK, status);
    ...
    public void testDynamicSignalDef_registerHandlers_withBogusRule() throws Exception {
        ...
        DynamicBusObject busObjHandler = new DynamicEmitter(
            clientBus, buildBusObjectInfo("",rule)); // empty path, bogus rule
        status = clientBus.registerSignalHandlers(busObjHandler);
        assertEquals(Status.OK, status);
