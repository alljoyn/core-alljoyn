...
public class DoorSessionManager {
    ...
    public Door getProxyDoorObject(String busName) throws BusException {
        if (sessionMap.containsKey(busName)) {
            ...
            Status status = mBus.joinSession(busName, DOOR_APPLICATION_PORT, sessionId, sessionOpts, new SessionListener());
            if (status != Status.OK) {
                return null;
            }
            ...
            ProxyBusObject mProxyObj = mBus.getProxyBusObject(busName, Door.DOOR_PATH, sessionId.value, new Class<?>[] {Door.class});
